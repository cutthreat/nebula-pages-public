(() => {
  ...
  const setBackgroundInert = (isInert) => {
    const modalOwner = root.parentElement;
    if (!modalOwner) return;
    [...modalOwner.children].forEach((node) => {
      if (node === root) return;
  ...
  const close = (reason = 'dismissed') => {
    if (testPaymentPhase === 'processing' || testPaymentPhase === 'error') return;
  ...
  confirm.addEventListener('click', () => {
    if (timer) return;
    const detail = selectionDetail('confirm');
    root.dispatchEvent(new CustomEvent('nebula:credit-topup-confirm-required', { bubbles: true, detail }));
    announce('Opening order details. Apple Pay test payments update only your local demo balance; no real money is charged.');
    // The checkout panel is a body sibling. Release the first dialog's focus lock
    // before handing control to it, otherwise the panel would be visible but inert.
    ...
    root.dispatchEvent(new CustomEvent('nebula:credit-topup-checkout-requested', { bubbles: true, detail: { ...detail, opener } }));
    if (!document.querySelector('[data-credit-topup-checkout-flow]')) {
      announce('Checkout preview is available from Billing. No payment or balance change was made.');
    }
  });
  ...
  document.addEventListener('nebula:credit-topup-test-payment', (event) => {
    const detail = event.detail || {};
    if (!['processing', 'success', 'error'].includes(detail.phase)) return;
    const wasOpen = !root.hidden;
    testPaymentPhase = detail.phase;
    opener = detail.opener instanceof HTMLElement ? detail.opener : opener;
    currentBalance = Number.isFinite(detail.balance) ? detail.balance : firstPageBalance();
    syncBalance();
    setStep(detail.phase);
    root.hidden = false; scrim.hidden = false; root.classList.add('is-open');
    document.documentElement.classList.add('nebula-modal-open');
    if (!wasOpen) setBackgroundInert(true);
    root.querySelectorAll('[data-credit-topup-action="close"]').forEach((button) => { button.disabled = detail.phase !== 'success'; });
    if (detail.phase === 'success' && successCopy) {
      successCopy.textContent = `${detail.credits} credits added. Your balance is ${detail.balance} credits. Saved in this local test account. No real payment was made.`;
    }
    announce(detail.phase === 'processing' ? 'Processing an Apple Pay test payment. No real charge.' : detail.phase === 'success' ? successCopy.textContent : 'The test payment receipt is not confirmed. Retry safely to check the same order.');
    window.requestAnimationFrame(() => (root.querySelector(`[data-credit-topup-step="${detail.phase}"] button`) || dialog).focus());
  });
  root.querySelector('[data-credit-topup-action="retry"]')?.addEventListener('click', () => {
    if (testPaymentPhase === 'error') document.dispatchEvent(new CustomEvent('nebula:credit-topup-test-payment-retry'));
  });
})();
