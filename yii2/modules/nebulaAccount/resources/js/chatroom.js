(() => {
  ...
  // Notification destination has its own navigation-only owner, never a messenger lifecycle.
  if (!root || root.dataset.c76State === 'notification_destination') return;

  const assetBase = root.dataset.assetBase || '';
  const miaDialogueDemo = root.dataset.miaDialogueDemo === 'true';
  ...
  const defaultOpenMarkup = messagesNode?.dataset.defaultIntro !== undefined ? messagesNode.innerHTML : '';
  const isDefaultOpen = root.dataset.c76State === 'default_open';
  const initialConversationId = root.dataset.selectedExpertId || 'margo-lover';
  ...
  const consultationCluster = root.querySelector('[data-consultation-cluster]');
  const consultationTimerNode = root.querySelector('[data-consultation-timer]');
  const consultationConfirmation = root.querySelector('[data-consultation-confirmation]');
  ...
  const conversationGroups = Array.from(root.querySelectorAll('[data-conversation-group]'));
  ...
  const mediaThumbnails = root.querySelector('[data-media-viewer-thumbnails]');
  ...
  const localMessageId = () => `local-${window.crypto?.randomUUID?.() || `${Date.now()}-${Math.random().toString(16).slice(2)}`}`;
  let voiceTimer = null;
  let voiceSeconds = 0;
  let voiceIsPaused = false;
  let voiceIsOneTime = false;
  let consultationSeconds = 0;
  let consultationState = 'idle';
  let consultationMinuteProgress = 0;
  let consultationPendingIntent = null;
  let consultationOpener = null;
  let consultationConfirmedState = 'idle';
  let consultationDialogMode = 'start';
  let consultationInertBackground = [];
  const consultationByConversation = new Map();
  // Availability notifications and calendar reservations are independent
  // commitments. Keep them per conversation so switching experts never
  // turns a confirmed reminder/booking into a fresh paid-chat action.
  const consultationCommitmentsByConversation = new Map();
  const makeConsultationCommitment = (kind, expertName = '') => ({
    kind, expertName, phase: 'off', confirmed: false, operation: null,
    requestId: null, idempotencyKey: null, subscriptionId: null,
    bookingId: null, booking: null, previousPhase: 'off',
  });
  const commitmentBucketFor = (conversationId) => {
    if (!consultationCommitmentsByConversation.has(conversationId)) {
      consultationCommitmentsByConversation.set(conversationId, {
        notify: makeConsultationCommitment('notify'),
        booking: makeConsultationCommitment('booking'),
      });
    }
    const bucket = consultationCommitmentsByConversation.get(conversationId);
    if (!bucket.notify) bucket.notify = makeConsultationCommitment('notify');
    if (!bucket.booking) bucket.booking = makeConsultationCommitment('booking');
    return bucket;
  };
  const commitmentEntryFor = (conversationId, kind, expertName = '') => {
    const normalizedKind = kind === 'book' ? 'booking' : kind;
    if (!conversationId || !['notify', 'booking'].includes(normalizedKind)) return null;
    const entry = commitmentBucketFor(conversationId)[normalizedKind];
    if (expertName) entry.expertName = expertName;
    return entry;
  };
  const commitmentVisualFor = (conversationId) => {
    const bucket = commitmentBucketFor(conversationId);
    const entries = [bucket.notify, bucket.booking];
    const priority = (entry) => (entry.kind === 'booking' ? 1 : 0);
    const pendingEntries = entries.filter((entry) => entry.phase === 'pending').sort((a, b) => priority(b) - priority(a));
    const activeEntries = entries.filter((entry) => entry.confirmed === true || entry.phase === 'confirmed').sort((a, b) => priority(b) - priority(a));
    const retainedEntries = entries.filter((entry) => entry.phase !== 'off').sort((a, b) => priority(b) - priority(a));
    const entry = pendingEntries[0] || activeEntries[0] || retainedEntries[0] || null;
    return {
      entry,
  ...
  rows.forEach((row) => {
    ...
    const portrait = row.querySelector('.c76-thread__avatar:not(.c76-thread__avatar--saved)');
    rowProfiles[name] = {
      avatar: portrait?.src || `${assetBase}/images/chatroom/margo-header.webp`,
      avatarWebp: portrait?.closest('picture')?.querySelector('source[type="image/webp"]')?.srcset || '',
  ...
  // Only profiles with a real Yii route may link out of the chat details card.
  // The other conversation fixtures remain inside the details dialog instead of
  // claiming a profile page that the current source does not provide.
  const routedPsychicProfiles = {
    'Margo Lover': 'margo-lover',
  };
  ...
  const conversations = rows
    ...
    .map((row) => {
      const name = row.dataset.conversationName;
      const seed = row.dataset.conversationSelection === 'true' ? {
        presence: rowProfiles[name]?.presence || 'Offline',
        rate: `${row.dataset.conversationRate} credits/min`,
        topic: '', messages: [],
      } : conversationSeed[name] || {
      ...
      return {
        id: row.dataset.conversationId || slug(name), name, row, profileSlug: row.dataset.conversationProfileSlug || '', avatar: rowProfiles[name]?.avatar, avatarWebp: rowProfiles[name]?.avatarWebp || '', unread: rowProfiles[name]?.unread || 0,
  ...
  const defaultConversation = conversations.find((conversation) => conversation.id === initialConversationId);
  if (isDefaultOpen && defaultConversation) {
    // source: Figma C76 1373:28806 Default Chatroom — the opening state is
    // pre-chat, so it has intro cards and no active-history message/pin state.
    defaultConversation.messages = [];
    // Availability comes from the expert row, never from the intro/history layout.
    defaultConversation.pinnedId = null;
  }
  const seedSavedMessages = conversations.filter((conversation) => conversation.messages.length > 0).slice(0, 2).map((conversation, index) => {
  ...
  const state = { activeId: initialConversationId, filter: 'all', saved: seedSavedMessages, search: '', defaultOpen: isDefaultOpen };
  const activeConversation = () => conversations.find((conversation) => conversation.id === state.activeId) || conversations[0];
  // Page-local reading/draft continuity. No storage, server receipt or draft persistence.
  const conversationViews = new Map();
  const jumpMessagesTo = (top) => {
    if (!messagesNode) return;
    // CSS smooth scrolling is for user navigation. A receipt/render must not
    // interrupt an in-flight animation and preserve an obsolete position.
    const previous = messagesNode.style.scrollBehavior;
    messagesNode.style.scrollBehavior = 'auto';
    messagesNode.scrollTop = top;
    if (previous) messagesNode.style.scrollBehavior = previous;
    else messagesNode.style.removeProperty('scroll-behavior');
  };
  const rememberConversationView = () => {
    if (!messagesNode?.clientHeight || threadPanel?.inert || workspace?.classList.contains('is-list-view')) return;
    const viewport = messagesNode.getBoundingClientRect();
    const anchor = Array.from(messagesNode.querySelectorAll('[data-message-id]')).find(message => message.getBoundingClientRect().bottom > viewport.top + 1);
    const bottom = Math.max(0, messagesNode.scrollHeight - messagesNode.clientHeight - messagesNode.scrollTop);
    conversationViews.set(state.activeId, {
  ...
  const restoreConversationView = () => {
    const view = conversationViews.get(state.activeId);
    if (!view) return;
    if (view.draft === input.value) {
      input.setSelectionRange(...view.selection);
      input.scrollTop = view.inputScrollTop;
    }
    const anchor = Array.from(messagesNode.querySelectorAll('[data-message-id]')).find(message => message.dataset.messageId === view.anchorId);
    if (view.bottom < 48) jumpMessagesTo(messagesNode.scrollHeight);
    else if (anchor) jumpMessagesTo(messagesNode.scrollTop + anchor.getBoundingClientRect().top - messagesNode.getBoundingClientRect().top - view.offset);
    else jumpMessagesTo(Math.max(0, messagesNode.scrollHeight - messagesNode.clientHeight - view.bottom));
  };
  ...
  const closePopovers = (restoreFocus = false) => {
    root.querySelectorAll('.c76-chat-search,.c76-chat-more-menu,.c76-message-menu,.c76-emoji-picker,.c76-attachment-menu,.c76-filter-menu').forEach((node) => { node.hidden = true; });
    if (threadSearch?.value) resetThreadSearch();
    root.querySelectorAll('.c76-reaction-picker').forEach((node) => node.remove());
    root.querySelectorAll('[aria-expanded="true"][data-chat-action],[data-message-action="more"],[data-composer-action],[data-conversation-filter-toggle]').forEach((button) => button.setAttribute('aria-expanded', 'false'));
  ...
  const reactToMessage = (conversation, message, emoji) => {
    const selected = changeReaction(message, emoji);
    root.dispatchEvent(new CustomEvent('nebula:chat-message-reaction-intent', { bubbles: true, detail: { conversationId: conversation.id, messageId: message.id, reaction: selected ? emoji : '', selected, persisted: false, localProjection: true } }));
    closePopovers();
    renderMessages({ keepScroll: true });
    announce(`${emoji} reaction ${selected ? 'added' : 'removed'} locally. Not delivered to the host.`);
    const nextArticle = messagesNode?.querySelector(`[data-message-id="${CSS.escape(message.id)}"]`);
    (nextArticle?.querySelector(`[data-reaction="${CSS.escape(emoji)}"]`) || nextArticle?.querySelector('[data-message-action="more"]'))?.focus();
  };
  ...
  const openReactionPicker = (article, opener, message) => {
    ...
    root.append(picker);
    popoverOpener = article.querySelector('[data-message-action="more"]') || opener;
    popoverOpener.setAttribute('aria-expanded', 'true');
    ...
    picker.querySelectorAll('[data-picker-reaction]').forEach((button) => button.addEventListener('click', () => {
      reactToMessage(conversation, message, button.dataset.pickerReaction);
    }));
  };
  ...
  const updateComposer = () => {
    ...
    const contentHeight = input.value.length === 0
      ? parseFloat(window.getComputedStyle(input).lineHeight) || 16
    ...
    const voicePreviewOpen = Boolean(voiceRecorder && !voiceRecorder.hidden);
    const ready = voicePreviewOpen || input.value.trim().length > 0 || Boolean(conversation.attachment);
    const multiline = input.value.includes('\n') || input.scrollHeight > 43;
    sendButton.disabled = !ready;
    sendButton.setAttribute('aria-disabled', ready ? 'false' : 'true');
    sendButton.setAttribute('aria-label', voicePreviewOpen ? 'Send voice message preview' : conversation.editId ? 'Save message changes' : 'Send message');
    const stateName = voicePreviewOpen ? 'voice_recording' : ready ? (multiline ? 'multiline' : 'text_ready') : 'empty_unactive';
    iconStrip.dataset.composerState = stateName;
    const imageState = stateName === 'voice_recording' ? 'empty' : stateName === 'multiline' ? 'multiline' : (stateName === 'text_ready' ? 'ready' : 'empty');
  ...
  const deliveryMarkup = (message) => {
    ...
    if (message.deliveryState === 'sending') {
      return '<span class="c76-message__status" data-message-status data-status="sending" data-local-preview="true" aria-label="Sending; awaiting an authoritative server receipt" title="Awaiting server receipt"><span>Sending…</span></span>';
    }
    if (message.deliveryState === 'failed') {
      return '<span class="c76-message__status" data-message-status data-status="failed" aria-label="Message was not accepted by the integration host" title="Not accepted"><span>Failed</span></span>';
    }
    const stateName = ['sent', 'delivered', 'read'].includes(message.deliveryState) ? message.deliveryState : 'sending';
    if (stateName === 'sending') {
      return '<span class="c76-message__status" data-message-status data-status="sending" data-local-preview="true" aria-label="Sending; awaiting an authoritative server receipt" title="Awaiting server receipt"><span>Sending…</span></span>';
    }
    const label = stateName[0].toUpperCase() + stateName.slice(1);
    if (message.serverReceipt === true && stateName === 'sent') {
      return `<span class="c76-message__status" data-message-status data-status="sent" data-sandbox-receipt="true" aria-label="Sent; accepted by the local integration host, delivery unproven" title="Accepted by local integration host"><span>${label}</span><svg aria-hidden="true" focusable="false"><use href="${assetBase}/images/chatroom/message-status-sprite.svg#sent"></use></svg></span>`;
    }
  ...
  const renderMessages = ({ keepScroll = false } = {}) => {
    const conversation = activeConversation();
    if (!messagesNode) return;
    root.dataset.messengerLive = 'true';
    if (state.defaultOpen && conversation.id === initialConversationId) {
      messagesNode.innerHTML = defaultOpenMarkup;
      readyCard.hidden = false;
      messagesNode.append(readyCard);
      renderPinned();
      applyThreadSearch();
      return;
    }
    // Intro and history share controls/layout; only their message content differs.
    ...
    if (!readyCard.hidden) messagesNode.append(readyCard);
    renderPinned();
    applyThreadSearch();
    jumpMessagesTo(keepScroll ? Math.max(0, messagesNode.scrollHeight - messagesNode.clientHeight - priorBottom) : messagesNode.scrollHeight);
  };
  ...
  const renderPinned = () => {
    ...
    if (state.defaultOpen && conversation.id === initialConversationId) {
      ...
      pinnedBanner.dataset.pinnedMessageId = '';
      return;
    }
  ...
  const renderList = () => {
    ...
    let visible = 0;
    rows.forEach((row) => {
      if (row.classList.contains('c76-thread--saved')) { const selected = savedView?.hidden === false; row.hidden = Boolean(query) || state.filter !== 'all'; row.classList.toggle('is-active', selected); row.setAttribute('aria-current', String(selected)); return; }
      const conversation = conversations.find((item) => item.name === row.dataset.conversationName);
      const listEntry = conversation || (row.getAttribute('aria-disabled') === 'true' ? rowProfiles[row.dataset.conversationName] : null);
      if (!listEntry) { row.hidden = true; return; }
      const filterMatch = state.filter === 'all' || (state.filter === 'unread' && listEntry.unread > 0) || (state.filter === 'online' && listEntry.online) || (state.filter === 'favorites' && listEntry.favorite) || (state.filter === 'muted' && listEntry.muted);
      const queryMatch = !query || `${row.dataset.conversationName} ${listEntry.topic || ''}`.toLowerCase().includes(query);
      row.hidden = !(filterMatch && queryMatch);
      if (!row.hidden) visible += 1;
      if (!conversation) return;
      const selected = conversation.id === state.activeId && savedView?.hidden !== false;
    ...
    conversationGroups.forEach((group) => {
      group.hidden = !group.querySelector('[data-conversation-row]:not([hidden])');
    });
  ...
  const setPortraitSource = (image, conversation, alt = '') => {
    if (!image) return;
    const source = image.closest('picture')?.querySelector('source[type="image/webp"]');
    if (source) {
      if (conversation.avatarWebp) source.srcset = conversation.avatarWebp;
  ...
  const renderConversation = ({ focus = true } = {}) => {
    ...
    setPortraitSource(avatar, conversation);
    ...
    root.querySelectorAll('[data-consultation-expert-name]').forEach((node) => { node.textContent = conversation.name; });
    root.querySelectorAll('[data-consultation-rate]').forEach((node) => { node.textContent = conversation.rate; });
    renderConsultationControl();
    const identity = root.querySelector('.c76-chat-identity');
    identity?.setAttribute('aria-label', `View details for ${conversation.name}`);
    const psychicSlug = conversation.profileSlug || routedPsychicProfiles[conversation.name];
    root.querySelectorAll('[data-expert-profile-link]').forEach((link) => {
      if (psychicSlug) {
        link.href = `/nebula-account/psychics/${psychicSlug}`;
        link.setAttribute('aria-label', `Open ${conversation.name} profile`);
        link.textContent = 'Open profile →';
      } else {
        link.href = '/nebula-account/psychics#all-psychics';
        link.setAttribute('aria-label', 'Browse available psychics');
        link.textContent = 'Browse psychics →';
      }
    });
    root.querySelectorAll('[data-expert-details-modal] h2').forEach((heading) => { heading.textContent = conversation.name; });
    root.querySelectorAll('[data-expert-details-modal] .expert-card__portrait,[data-expert-details-modal] .mobile-card__portrait').forEach((image) => setPortraitSource(image, conversation, conversation.name));
    ...
    renderMessages(); renderReply(); renderEdit(); renderAttachment();
    closeConversationSearch({ restoreFocus: false }); updateComposer();
    restoreConversationView();
    if (focus) { name?.setAttribute('tabindex', '-1'); name?.focus({ preventScroll: true }); }
  ...
  const syncConversationSearch = () => {
    ...
    const sourceIdle = isDefaultOpen && !compact.matches && !open;
    searchShell.classList.toggle('is-source-idle', sourceIdle);
    searchShell.classList.toggle('is-search-open', open);
    searchTrigger.closest('.c76-search__field')?.classList.toggle('is-search-trigger', !open && !sourceIdle);
    searchTrigger.hidden = open;
    searchTrigger.tabIndex = sourceIdle ? -1 : 0;
    searchTrigger.setAttribute('aria-expanded', String(open));
    if (searchInput) searchInput.tabIndex = !open && !sourceIdle ? -1 : 0;
    if (searchClear) searchClear.hidden = !open;
  };
  ...
  const focusConversationList = () => {
    const row = activeConversation()?.row;
    if (row && !row.hidden) row.focus();
    else filterToggle?.focus();
  };

  const closeSavedView = ({ restoreFocus = false } = {}) => {
    const wasOpen = savedView?.hidden === false;
    ...
    if (compact.matches && restoreFocus) {
      workspace?.classList.add('is-list-view');
      syncConversationScrollbar();
    }
    if (restoreFocus) focusConversationList();
  };
  ...
  const selectConversation = (conversation, row, focus = true) => {
    ...
    if (state.activeId !== conversation.id) {
      closeConsultationConfirmation({ restoreFocus: false });
      consultationByConversation.set(state.activeId, { consultationState, consultationConfirmedState, consultationSeconds, consultationMinuteProgress, consultationPendingIntent });
      const saved = consultationByConversation.get(conversation.id);
      consultationState = saved?.consultationState || 'idle';
      consultationConfirmedState = saved?.consultationConfirmedState || 'idle';
      consultationSeconds = saved?.consultationSeconds || 0;
      consultationMinuteProgress = saved?.consultationMinuteProgress || 0;
      consultationPendingIntent = saved?.consultationPendingIntent || null;
    }
  ...
  const openSavedView = () => {
    if (!savedView || !threadPanel) return;
    rememberConversationView();
    renderSaved();
    workspace?.classList.remove('is-list-view');
    savedView.hidden = false;
    renderList();
    threadPanel.inert = true;
    threadPanel.setAttribute('inert', '');
    threadPanel.setAttribute('aria-hidden', 'true');
    savedView.querySelector('[data-saved-back]')?.focus();
  };
  rows.forEach((row) => {
    if (row.classList.contains('c76-thread--saved')) {
      row.addEventListener('click', openSavedView); return;
    }
    const conversation = conversations.find((item) => item.name === row.dataset.conversationName);
    if (!conversation) return;
  ...
  savedList?.addEventListener('keydown', (event) => {
    const item = event.target.closest('[data-saved-open]');
    if (item && event.target === item && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); const saved = state.saved.find((entry) => entry.key === item.dataset.savedOpen); if (saved) openSavedMessage(saved); }
  });
  const syncCompactView = () => {
    if (!backButton) return;
    backButton.hidden = !compact.matches;
    backButton.tabIndex = compact.matches ? 0 : -1;
  ...
  // CSS can blur a control before the media-query callback. Keep its owner
  // across that transition, but never reclaim focus after a user clicks away.
  let responsiveFocus = null;
  document.addEventListener('focusin', event => { responsiveFocus = root.contains(event.target) ? event.target : null; });
  document.addEventListener('pointerdown', event => {
    if (responsiveFocus && !responsiveFocus.contains(event.target)) responsiveFocus = null;
  }, true);
  compact.addEventListener?.('change', () => {
    const previousFocus = responsiveFocus;
    if (compact.matches && previousFocus && menu?.contains(previousFocus)) {
      rememberConversationView();
      workspace?.classList.add('is-list-view');
      menu.classList.add('is-expanded');
    }
    if (!compact.matches) menu?.classList.remove('is-expanded');
    syncCompactView(); syncConversationSearch(); syncConversationScrollbar();
    if (!compact.matches && previousFocus === backButton) {
      const name = root.querySelector('[data-chat-name]');
      name?.setAttribute('tabindex', '-1');
      name?.focus({ preventScroll: true });
    }
    else if (previousFocus?.isConnected && previousFocus.getBoundingClientRect().width) previousFocus.focus({ preventScroll: true });
  });
  ...
  root.addEventListener('nebula:account-menu-opening', () => closePopovers());
  ...
  const renderChatMute = () => {
    const button = chatMenu?.querySelector('[data-chat-menu="mute"]');
    if (!button) return;
    const muted = Boolean(activeConversation().muted);
    button.setAttribute('aria-checked', muted ? 'true' : 'false');
    const label = button.querySelector('[data-chat-menu-label]');
    if (label) label.textContent = muted ? 'Muted' : 'Mute';
  };
  renderChatMute();
  actionButton('more')?.addEventListener('click', () => { closePopovers(); popoverOpener = actionButton('more'); const panel = root.querySelector('.c76-chat-more-menu'); renderChatMute(); panel.hidden = false; actionButton('more').setAttribute('aria-expanded', 'true'); panel.querySelector('button')?.focus(); });
  ...
  messagesNode?.addEventListener('click', async (event) => {
    ...
    if (control.dataset.messageMenu && message) {
      ...
      if (action === 'react') { event.stopPropagation(); openReactionPicker(article, control, message); return; }
      if (action === 'save') {
        ...
        root.dispatchEvent(new CustomEvent('nebula:chat-message-bookmark-intent', { bubbles: true, detail: { conversationId: conversation.id, messageId: message.id, bookmarked, persisted: false, localProjection: true } }));
        renderMessages({ keepScroll: true });
        renderSaved();
      }
      if (action === 'copy') {
        try {
          ...
          await navigator.clipboard.writeText(message.text || message.media?.title || '');
          showToast('Copied');
        } catch (_) { showToast('Copy unavailable'); }
      }
      if (action === 'pin') {
        ...
        root.dispatchEvent(new CustomEvent('nebula:chat-message-pin-intent', { bubbles: true, detail: { conversationId: conversation.id, messageId: message.id, pinned: next, persisted: false, localProjection: true } }));
        renderMessages({ keepScroll: true });
    ...
    if (control.dataset.reaction && message) {
      reactToMessage(conversation, message, control.dataset.reaction);
      return;
    }
    if (control.dataset.mediaOpen && message?.media) {
      const items = message.media.kind === 'bundle' ? (message.media.items || []) : [message.media];
      const index = Math.min(Math.max(0, Number(control.dataset.mediaIndex) || 0), Math.max(0, items.length - 1));
      if (items[index]) openMedia(items, index, control);
      return;
    }
  ...
  root.querySelector('[data-pinned-close]')?.addEventListener('click', () => {
    ...
    if (current) {
      ...
      root.dispatchEvent(new CustomEvent('nebula:chat-message-pin-intent', { bubbles: true, detail: { conversationId: conversation.id, messageId: current.id, pinned: false, persisted: false, localProjection: true } }));
    }
    conversation.pinnedId = null;
    renderMessages({ keepScroll: true });
  ...
  const sendLocal = () => {
    ...
    if (miaDialogueDemo && conversation.id === 'mia-jacomo') {
      if (!text) return;
      document.dispatchEvent(new CustomEvent('nebula:mia-demo-message', { detail: { text } }));
      return;
    }
    ...
    if (editedMessage) {
      ...
      conversation.preEditDraft = '';
      input.value = conversation.draft;
      root.dispatchEvent(new CustomEvent('nebula:chat-message-edited-local', { bubbles: true, detail: { conversationId: conversation.id, messageId: editedMessage.id, text: editedMessage.text, deliveryState: editedMessage.deliveryState, localOnly: true, persisted: false, serverReceipt: false } }));
    ...
    const options = { id: localMessageId(), time: new Date().toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }), deliveryState: 'sending', local: true, reply: reply?.text?.slice(0, 120) };
    ...
    // A voice note has no text or image attachment.  Send its metadata through
    // the same integration DTO so the local contract can validate it as media
    // instead of incorrectly rejecting the message as empty.
    const integrationMedia = message.media || (message.voice ? { kind: 'voice', duration: message.voice.duration } : null);
    root.dispatchEvent(new CustomEvent('nebula:chat-message-created-local', { bubbles: true, detail: { conversationId: conversation.id, messageId: message.id, text: message.text || '', media: integrationMedia, deliveryState: 'sending', localOnly: true, persisted: false, delivered: false, serverReceipt: false } }));
    ...
    announce('Message is waiting for an authoritative server receipt. Sent, Delivered and Read are never inferred locally.');
  };

  document.addEventListener('nebula:integration-receipt', (event) => {
    const detail = event.detail || {};
    if (detail.command !== 'chat.message.create') return;
    const conversation = conversations.find((item) => item.id === detail.payload?.conversationId);
    const message = conversation?.messages.find((item) => item.id === detail.payload?.clientMessageId);
    if (!message) return;
    message.deliveryState = 'sent';
    message.serverReceipt = true;
    message.serverId = detail.result?.message?.id || null;
    if (conversation.id === state.activeId) renderMessages({ keepScroll: true });
    announce('Message accepted by the local integration host. Delivery and read remain unproven.');
  });
  document.addEventListener('nebula:integration-error', (event) => {
    const detail = event.detail || {};
    if (detail.command !== 'chat.message.create') return;
    const conversation = conversations.find((item) => item.id === detail.payload?.conversationId);
    const message = conversation?.messages.find((item) => item.id === detail.payload?.clientMessageId);
    ...
    if (conversation.id === state.activeId) renderMessages({ keepScroll: true });
    announce(`Message was not accepted: ${detail.message || 'integration error'}`);
  });
  ...
  sendButton?.addEventListener('click', () => {
    if (voiceRecorder && !voiceRecorder.hidden) { sendVoiceRecording(); return; }
    sendLocal();
  });
  const showSendPlaneHover = () => {
    if (sendStateImage?.dataset.srcMultiline) sendStateImage.src = sendStateImage.dataset.srcMultiline;
  };
  sendButton?.addEventListener('mouseenter', showSendPlaneHover);
  sendButton?.addEventListener('focus', showSendPlaneHover);
  sendButton?.addEventListener('mouseleave', updateComposer);
  sendButton?.addEventListener('blur', updateComposer);

  const emojiPicker = root.querySelector('.c76-emoji-picker');
  root.querySelector('[data-composer-action="emoji"]')?.addEventListener('click', (event) => { closePopovers(); root.querySelectorAll('[data-emoji-picker-category]').forEach((item) => item.setAttribute('aria-pressed', String(item.dataset.emojiPickerCategory === 'recent'))); root.querySelectorAll('[data-emoji]').forEach((emoji) => { emoji.hidden = false; }); popoverOpener = event.currentTarget; emojiPicker.hidden = false; event.currentTarget.setAttribute('aria-expanded', 'true'); emojiPicker.querySelector('[data-emoji]')?.focus(); });
  ...
  root.querySelectorAll('[data-emoji-picker-category]').forEach((button) => button.addEventListener('click', () => {
    const category = button.dataset.emojiPickerCategory;
    root.querySelectorAll('[data-emoji-picker-category]').forEach((item) => item.setAttribute('aria-pressed', String(item === button)));
    root.querySelectorAll('[data-emoji]').forEach((emoji) => { emoji.hidden = category !== 'recent' && emoji.dataset.emojiCategory !== category; });
    emojiPicker.querySelector('[data-emoji]:not([hidden])')?.focus();
  }));
  ...
  attachmentInput?.addEventListener('change', () => {
    ...
    const localItems = files.map((candidate) => { const localObjectUrl = URL.createObjectURL(candidate); const type = validFile(candidate); return { kind: type.isVideo ? 'video' : type.isImage ? 'photo' : 'file', src: localObjectUrl, title: candidate.name, size: candidate.size, mime: candidate.type || 'application/octet-stream', localObjectUrl: true }; });
  ...
  const voiceOnceButton = root.querySelector('[data-voice-once]');
  const voicePauseButton = root.querySelector('[data-voice-pause]');
  ...
  const updateVoiceRecorder = () => {
    ...
    if (voiceRecorder) voiceRecorder.dataset.state = voiceRecorder.hidden ? 'ready' : voiceIsPaused ? 'paused' : 'recording';
    if (voiceOnceButton) {
      voiceOnceButton.setAttribute('aria-pressed', String(voiceIsOneTime));
      voiceOnceButton.classList.toggle('is-selected', voiceIsOneTime);
    }
    if (voicePauseButton) {
      voicePauseButton.setAttribute('aria-pressed', String(voiceIsPaused));
      voicePauseButton.setAttribute('aria-label', voiceIsPaused ? 'Resume voice message preview' : 'Pause voice message preview');
      voicePauseButton.classList.toggle('is-paused', voiceIsPaused);
    }
  };
  ...
  const startVoiceRecording = () => {
    if (activeConversation().editId) { input.focus(); return; }
    ...
    updateVoiceRecorder();
    updateComposer();
    window.setTimeout(() => root.querySelector('[data-voice-cancel]')?.focus(), 0);
    announce('Local voice preview started. Use discard, pause, single-play, or send. No microphone is used.');
  };
  const toggleVoiceOnce = () => {
    if (voiceRecorder.hidden) return;
    voiceIsOneTime = !voiceIsOneTime;
    updateVoiceRecorder();
    announce(voiceIsOneTime ? 'Voice preview is marked for one playback.' : 'Voice preview is marked for normal playback.');
  };
  const toggleVoicePause = () => {
    if (voiceRecorder.hidden) return;
    voiceIsPaused = !voiceIsPaused;
    if (voiceIsPaused) clearVoiceTimer(); else runVoiceTimer();
    updateVoiceRecorder();
    announce(voiceIsPaused ? 'Voice preview paused.' : 'Voice preview resumed.');
  };
  const sendVoiceRecording = () => {
    const conversation = activeConversation();
    if (conversation.editId) { cancelVoiceRecording({ restoreFocus: false }); input.focus(); return; }
    const draft = input.value;
    const duration = formatVoiceTime(Math.max(1, voiceSeconds));
    const singlePlay = voiceIsOneTime;
    closeVoiceRecorder();
    input.value = '';
    conversation.attachment = { kind: 'voice', title: 'Voice message preview', duration, singlePlay };
    root.dispatchEvent(new CustomEvent('nebula:chat-voice-preview-send-intent-required', {
      bubbles: true,
      detail: {
        duration,
        singlePlay,
  ...
  voiceOnceButton?.addEventListener('click', toggleVoiceOnce);
  voicePauseButton?.addEventListener('click', toggleVoicePause);
  ...
  const consultationHostStates = new Set(['idle', 'pending_start', 'active', 'pending_pause', 'paused', 'pending_resume', 'pending_end', 'ended']);
  // Explicit owner-local prototype adapter. Never emit service/billing intents in demo mode.
  const consultationDemo = !miaDialogueDemo && root.dataset.consultationDemo === 'true' && root.dataset.staticProjection === 'true'
    && ['127.0.0.1', 'localhost', '[::1]'].includes(window.location.hostname);
  const consultationDemoSessions = new Map();
  let consultationDemoClock = null;
  let pendingDemoConnection = null;
  let demoConnectionSequence = 0;
  const demoElapsedMs = (session) => session.elapsedMs + (session.state === 'active' ? Math.max(0, performance.now() - session.startedAt) : 0);
  const renderDemoSessions = () => {
    consultationDemoSessions.forEach((session, conversationId) => {
      const elapsedSeconds = Math.floor(demoElapsedMs(session) / 1000);
      applyConsultationHostSnapshot({ role: 'client', conversationId, state: session.state, elapsedSeconds, minuteProgress: (elapsedSeconds % 60) / 60 });
    });
  };
  const syncDemoClock = () => {
    const running = Array.from(consultationDemoSessions.values()).some((session) => session.state === 'active');
    if (running && consultationDemoClock === null) consultationDemoClock = window.setInterval(renderDemoSessions, 250);
    if (!running && consultationDemoClock !== null) { window.clearInterval(consultationDemoClock); consultationDemoClock = null; }
  };
  const applyDemoIntent = (intent) => {
    const session = consultationDemoSessions.get(state.activeId) || { state: 'idle', elapsedMs: 0, startedAt: performance.now() };
    session.elapsedMs = demoElapsedMs(session);
    session.startedAt = performance.now();
    session.state = { start: 'active', resume: 'active', pause: 'paused', end: 'ended' }[intent];
    consultationDemoSessions.set(state.activeId, session);
    renderDemoSessions();
    syncDemoClock();
    announce('Demo consultation ' + session.state + '. No credits charged.');
  };
  const consultationProjectionState = () => consultationPendingIntent ? `pending_${consultationPendingIntent}` : consultationState;
  const consultationProjectionFor = (conversationId) => conversationId === state.activeId
    ? { consultationState, consultationConfirmedState, consultationSeconds, consultationMinuteProgress, consultationPendingIntent }
    : consultationByConversation.get(conversationId) || { consultationState: 'idle', consultationConfirmedState: 'idle', consultationSeconds: 0, consultationMinuteProgress: 0, consultationPendingIntent: null };
  const validateConsultationHostSnapshot = (snapshot) => {
    if (!snapshot || typeof snapshot !== 'object') return { valid: false, reason: 'malformed' };
    if (snapshot.role !== 'client') return { valid: false, reason: 'role-mismatched' };
    if (typeof snapshot.conversationId !== 'string' || !conversations.some((conversation) => conversation.id === snapshot.conversationId)) return { valid: false, reason: 'conversation-mismatched' };
    if (!consultationHostStates.has(snapshot.state)) return { valid: false, reason: 'malformed-state' };
    if (!Number.isInteger(snapshot.elapsedSeconds) || snapshot.elapsedSeconds < 0) return { valid: false, reason: 'malformed-elapsed' };
    if (!Number.isFinite(snapshot.minuteProgress) || snapshot.minuteProgress < 0 || snapshot.minuteProgress > 1) return { valid: false, reason: 'malformed-minute-progress' };
    if (snapshot.elapsedSeconds < consultationProjectionFor(snapshot.conversationId).consultationSeconds) return { valid: false, reason: 'regressive' };
    return { valid: true };
  };
  const applyConsultationHostSnapshot = (snapshot) => {
    const validation = validateConsultationHostSnapshot(snapshot);
    if (!validation.valid) {
      root.dispatchEvent(new CustomEvent('nebula:consultation-host-snapshot-rejected', { bubbles: true, detail: { reason: validation.reason, backendRequired: true, sessionMutated: false, billingMutated: false, persisted: false, staticProjection: true, liveMount: false } }));
      return false;
    }
    const previous = consultationProjectionFor(snapshot.conversationId);
    const next = {
      consultationState: snapshot.state,
      consultationConfirmedState: snapshot.state.startsWith('pending_') ? previous.consultationConfirmedState : snapshot.state,
      consultationSeconds: snapshot.elapsedSeconds,
      consultationMinuteProgress: snapshot.minuteProgress,
      consultationPendingIntent: null,
    };
    consultationByConversation.set(snapshot.conversationId, next);
    if (snapshot.conversationId === state.activeId) {
      if (snapshot.state !== consultationState) closeConsultationConfirmation();
      ({ consultationState, consultationConfirmedState, consultationSeconds, consultationMinuteProgress, consultationPendingIntent } = next);
      renderConsultationControl();
    }
    return true;
  };
  const commitmentActionHint = (entry) => {
    if (!entry) return '';
    if (entry.phase === 'pending') return 'Please wait';
    if (entry.kind === 'notify') {
      if (entry.confirmed) return entry.phase === 'error' ? 'Notification still on' : 'Cancel notification';
      return entry.phase === 'indeterminate' ? 'Check notification status' : entry.phase === 'error' ? 'Try notification again' : 'Notify when available';
    }
    if (entry.confirmed) return entry.phase === 'error' ? 'Booking still active' : 'Cancel booking';
    return entry.phase === 'indeterminate' ? 'Check booking status' : entry.phase === 'error' ? 'Try booking again' : 'Book consultation';
  };
  const commitmentControlLabel = (entry, actionHint) => {
    if (!entry) return actionHint;
    if (entry.phase === 'pending') return 'Please wait';
    if (entry.kind === 'notify') return entry.confirmed ? 'Notification active' : actionHint;
    return entry.confirmed ? 'Booking active' : actionHint;
  };
  const renderConsultationControl = () => {
    const projectedState = consultationProjectionState();
    const paidActive = consultationConfirmedState === 'active';
    const paused = consultationConfirmedState === 'paused';
    const ended = consultationState === 'ended';
    const paidPending = consultationPendingIntent !== null || consultationState.startsWith('pending_');
    const paidLifecycle = paidPending || paidActive || paused || ended || consultationConfirmedState !== 'idle' || consultationState !== 'idle';
    const commitmentVisual = commitmentVisualFor(state.activeId);
    const commitment = commitmentVisual.entry;
    const commitmentActive = commitmentVisual.active;
    const hasCommitment = Boolean(commitment);
    const presence = String(activeConversation()?.presence || '').toLowerCase();
    // Busy and offline are different product decisions: a busy expert gets a
    // reminder route, while an offline expert gets the calendar route. Existing
    // commitments stay actionable even when presence changes underneath them.
    const expertBusy = !paidLifecycle && !hasCommitment && presence === 'busy';
    const expertOffline = !paidLifecycle && !hasCommitment && presence === 'offline';
    const notificationOnly = !paidLifecycle && (commitment?.kind === 'notify' || expertBusy);
    const bookingOnly = !paidLifecycle && !notificationOnly && (commitment?.kind === 'booking' || expertOffline);
    const booking = { expertId: state.activeId, available: false, booked: false };
    if (bookingOnly && !commitment) document.dispatchEvent(new CustomEvent('nebula:consultation-booking-availability', { detail: booking }));
    const bookingUnavailable = bookingOnly && !commitment && !booking.available && !booking.booked;
    const pending = paidPending || (!paidLifecycle && commitmentVisual.pending);
    const active = paidActive || (!paidLifecycle && commitmentActive);
    // Presence only selects the offer route. The commitment kind is reserved
    // for a real pending/confirmed/error/indeterminate subscription or booking
    // so a passive Busy/Offline control keeps the neutral iPhone-like track.
    const commitmentKind = !paidLifecycle && commitment ? commitment.kind : '';
    // Canonical lifecycle: consent waits for a separate operator Start; ending
    // a service session is not a failure or the end of its free dialogue.
    const awaitingOperator = miaDialogueDemo && state.activeId === 'mia-jacomo' && root.dataset.miaDemoPhase === 'awaiting_start';
    const actionHint = pending ? (awaitingOperator ? 'Waiting for operator' : 'Please wait') : ended ? 'Consultation ended' : paused ? 'Resume consultation' : paidActive ? 'Pause or end' : commitmentActionHint(commitment) || (notificationOnly ? 'Notify when available' : bookingOnly ? (booking.booked ? 'View booking' : bookingUnavailable ? 'Unavailable' : 'Book consultation') : 'Start consultation');
    if (consultationCluster) {
      consultationCluster.setAttribute('data-consultation-state', projectedState);
      consultationCluster.setAttribute('data-consultation-commitment', paidLifecycle ? 'paid' : commitmentKind || 'none');
      consultationCluster.setAttribute('data-consultation-phase', commitment?.phase || (paidLifecycle ? projectedState : 'off'));
    }
    const statusNode = root.querySelector('[data-consultation-status]');
    if (statusNode) statusNode.textContent = actionHint;
    threadPanel?.setAttribute('data-consultation-visual-state', paidActive ? 'active' : paused ? 'paused' : 'idle');
    threadPanel?.style.setProperty('--c76-consultation-perimeter-angle', `${Math.round(consultationMinuteProgress * 360)}deg`);
    if (consultationTimerNode) {
      consultationTimerNode.hidden = !(paidActive || paused);
      consultationTimerNode.textContent = formatConsultationTime(consultationSeconds);
      consultationTimerNode.dateTime = `PT${consultationSeconds}S`;
    }
    consultationControls.forEach((control) => {
      const intent = paidActive ? 'manage' : paused ? 'resume' : notificationOnly ? (commitmentActive ? 'notify-cancel' : 'notify') : bookingOnly ? (commitmentActive ? 'book-cancel' : 'book') : 'start';
      const miaNotification = notificationOnly && miaDialogueDemo && activeConversation()?.id === 'mia-jacomo';
      const disabled = pending || ended || bookingUnavailable;
      control.dataset.consultationState = projectedState;
      control.dataset.consultationPhase = commitment?.phase || (paidLifecycle ? projectedState : 'off');
      control.dataset.consultationCommitment = paidLifecycle ? 'paid' : commitmentKind || 'none';
      control.dataset.expertBusy = String(expertBusy);
      control.dataset.expertOffline = String(expertOffline);
      control.dataset.consultationIntent = intent;
      control.setAttribute('aria-controls', miaNotification ? 'mia-availability-notification-dialog' : notificationOnly ? 'expert-card-action-dialog' : bookingOnly ? 'c76-booking-consultation' : 'c76-consultation-confirmation');
      control.disabled = disabled;
      control.setAttribute('aria-disabled', String(disabled));
      // Keep the action hint in the status/ARIA channel. A native title
      // tooltip overlaps the compact header label and makes the switch look
      // like two controls at once.
      control.removeAttribute('title');
      if (control.getAttribute('role') === 'switch') control.setAttribute('aria-checked', String(active));
      else control.setAttribute('aria-pressed', String(active || paused));
      control.setAttribute('aria-busy', String(pending));
      control.setAttribute('aria-expanded', String((consultationConfirmation && !consultationConfirmation.hidden) || root.querySelector('[data-booking-consultation]')?.hidden === false || root.querySelector('[data-mia-availability-notification]')?.hidden === false || root.querySelector('[data-expert-card-action-modal]')?.hidden === false));
      const label = control.querySelector('[data-consultation-label]');
      const controlLabel = notificationOnly || bookingOnly ? commitmentControlLabel(commitment, actionHint) : control.getAttribute('role') === 'switch' ? 'Paid consultation' : ended ? 'Paid consultation ended' : paidActive ? 'Manage consultation' : paused ? 'Resume consultation' : 'Start consultation';
      if (label) label.textContent = controlLabel;
      control.setAttribute('aria-label', controlLabel);
    });
  };
  const commitmentOperationFor = (detail = {}, fallback = '', kind = '') => {
    const raw = String(detail.operation || detail.action || fallback || '').toLowerCase();
    if (['cancel', 'cancelled', 'unsubscribe'].includes(raw)) return 'cancel';
    if (['reserve', 'booking', 'book'].includes(raw)) return 'reserve';
    if (['notify', 'subscribe', 'subscription'].includes(raw)) return kind === 'booking' ? 'reserve' : 'notify';
    return kind === 'booking' ? 'reserve' : 'notify';
  };
  const commitmentAccepted = (detail = {}) => detail.accepted === true || detail.confirmed === true || detail.ok === true || detail.hostReceipt === true;
  const commitmentMatches = (entry, detail = {}) => {
    if (!entry || entry.phase === 'off') return Boolean(detail.requestId);
    if (entry.requestId && detail.requestId && entry.requestId !== detail.requestId) return false;
    if (entry.idempotencyKey && detail.idempotencyKey && entry.idempotencyKey !== detail.idempotencyKey) return false;
    return true;
  };
  const clearConsultationCommitment = (entry) => {
    entry.phase = 'off'; entry.confirmed = false; entry.operation = null;
    entry.requestId = null; entry.idempotencyKey = null; entry.subscriptionId = null;
    entry.bookingId = null; entry.booking = null; entry.previousPhase = 'off';
  };
  const updateConsultationCommitmentIntent = (kind, detail = {}, fallback = '') => {
    const expertId = String(detail.expertId || '');
    if (!conversations.some((conversation) => conversation.id === expertId)) return;
    const entry = commitmentEntryFor(expertId, kind, detail.expertName || activeConversation()?.name || '');
    const operation = commitmentOperationFor(detail, fallback, entry.kind);
    if (entry.phase === 'pending' && entry.requestId && detail.requestId && entry.requestId === detail.requestId) return;
    if (entry.requestId && detail.requestId && entry.requestId !== detail.requestId) return;
    entry.previousPhase = entry.phase;
    entry.operation = operation;
    entry.phase = 'pending';
    entry.confirmed = operation === 'cancel' ? entry.confirmed : false;
    entry.requestId = detail.requestId || entry.requestId;
    entry.idempotencyKey = detail.idempotencyKey || entry.idempotencyKey;
    entry.subscriptionId = detail.subscriptionId || entry.subscriptionId;
    entry.bookingId = detail.bookingId || entry.bookingId;
    if (detail.booking) entry.booking = { ...detail.booking };
    if (state.activeId === expertId) renderConsultationControl();
  };
  const updateConsultationCommitmentReceipt = (kind, detail = {}, fallback = '') => {
    const expertId = String(detail.expertId || '');
    if (!conversations.some((conversation) => conversation.id === expertId)) return;
    const entry = commitmentEntryFor(expertId, kind, detail.expertName || activeConversation()?.name || '');
    if (!commitmentMatches(entry, detail)) return;
    const operation = commitmentOperationFor(detail, fallback, entry.kind);
    if (!commitmentAccepted(detail)) {
      entry.phase = 'error'; entry.operation = operation; entry.previousPhase = entry.confirmed ? 'confirmed' : 'off';
      if (state.activeId === expertId) renderConsultationControl();
      return;
    }
    const cancelled = operation === 'cancel' || detail.transition === 'cancelled' || detail.cancelled === true || detail.subscribed === false || detail.booking === null;
    if (cancelled) clearConsultationCommitment(entry);
    else {
      entry.phase = 'confirmed'; entry.confirmed = true; entry.operation = null; entry.previousPhase = 'confirmed';
      entry.requestId = null; entry.idempotencyKey = null;
      if (entry.kind === 'notify') entry.subscriptionId = detail.subscriptionId || entry.subscriptionId || null;
  ...
  const updateConsultationCommitmentFailure = (kind, detail = {}, phase = 'error', fallback = '') => {
    const expertId = String(detail.expertId || '');
    if (!conversations.some((conversation) => conversation.id === expertId)) return;
    const entry = commitmentEntryFor(expertId, kind, detail.expertName || activeConversation()?.name || '');
    if (!commitmentMatches(entry, detail)) return;
    entry.previousPhase = entry.confirmed ? 'confirmed' : 'off';
    entry.phase = phase;
    entry.operation = commitmentOperationFor(detail, fallback, entry.kind);
    if (state.activeId === expertId) renderConsultationControl();
  };
  const abortConsultationCommitment = (kind, detail = {}) => {
    const expertId = String(detail.expertId || '');
    const entry = commitmentEntryFor(expertId, kind, detail.expertName || activeConversation()?.name || '');
    if (!conversations.some((conversation) => conversation.id === expertId) || !commitmentMatches(entry, detail)) return;
    const previous = ['confirmed', 'error', 'indeterminate'].includes(entry.previousPhase) ? entry.previousPhase : 'off';
    const wasConfirmed = previous === 'confirmed' || (previous !== 'off' && entry.confirmed);
    entry.phase = previous; entry.confirmed = wasConfirmed; entry.operation = null;
    entry.requestId = null; entry.idempotencyKey = null; entry.previousPhase = previous;
    if (state.activeId === expertId) renderConsultationControl();
  };
  [
    ['nebula:consultation-notification-intent', 'notify', 'notify'],
    ['nebula:consultation-notification-cancel-intent', 'notify', 'cancel'],
    ['nebula:expert-availability-subscription-requested', 'notify', 'notify'],
    ['nebula:consultation-booking-intent', 'booking', 'reserve'],
    ['nebula:consultation-booking-reserve-intent', 'booking', 'reserve'],
    ['nebula:consultation-booking-cancel-intent', 'booking', 'cancel'],
    ['nebula:expert-booking-requested', 'booking', 'reserve'],
    ['nebula:expert-booking-cancel-requested', 'booking', 'cancel'],
  ].forEach(([eventName, kind, fallback]) => document.addEventListener(eventName, (event) => updateConsultationCommitmentIntent(kind, event.detail || {}, fallback)));
  document.addEventListener('nebula:consultation-notification-receipt', (event) => updateConsultationCommitmentReceipt('notify', event.detail || {}, 'notify'));
  document.addEventListener('nebula:consultation-notification-cancel-receipt', (event) => updateConsultationCommitmentReceipt('notify', event.detail || {}, 'cancel'));
  document.addEventListener('nebula:expert-availability-subscription-receipt', (event) => updateConsultationCommitmentReceipt('notify', event.detail || {}, 'notify'));
  document.addEventListener('nebula:consultation-booking-receipt', (event) => updateConsultationCommitmentReceipt('booking', event.detail || {}, 'reserve'));
  document.addEventListener('nebula:consultation-booking-cancel-receipt', (event) => updateConsultationCommitmentReceipt('booking', event.detail || {}, 'cancel'));
  document.addEventListener('nebula:expert-booking-receipt', (event) => updateConsultationCommitmentReceipt('booking', event.detail || {}, 'reserve'));
  document.addEventListener('nebula:expert-booking-cancel-receipt', (event) => updateConsultationCommitmentReceipt('booking', event.detail || {}, 'cancel'));
  document.addEventListener('nebula:expert-card-action-receipt', (event) => {
    const detail = event.detail || {};
    const kind = detail.kind === 'book' || detail.kind === 'booking' || String(detail.action || '').includes('booking') ? 'booking' : 'notify';
    updateConsultationCommitmentReceipt(kind, detail, kind === 'booking' ? 'reserve' : 'notify');
  });
  document.addEventListener('nebula:consultation-notification-error', (event) => updateConsultationCommitmentFailure('notify', event.detail || {}, 'error', event.detail?.operation || 'notify'));
  document.addEventListener('nebula:consultation-notification-timeout', (event) => updateConsultationCommitmentFailure('notify', event.detail || {}, 'indeterminate', event.detail?.operation || 'notify'));
  document.addEventListener('nebula:expert-availability-subscription-error', (event) => updateConsultationCommitmentFailure('notify', event.detail || {}, 'error', 'notify'));
  document.addEventListener('nebula:expert-card-action-timeout', (event) => {
    const detail = event.detail || {};
    updateConsultationCommitmentFailure(detail.kind === 'book' ? 'booking' : 'notify', detail, 'indeterminate', detail.action);
  });
  document.addEventListener('nebula:consultation-booking-error', (event) => updateConsultationCommitmentFailure('booking', event.detail || {}, 'error', event.detail?.operation || event.detail?.kind || 'reserve'));
  document.addEventListener('nebula:consultation-booking-timeout', (event) => updateConsultationCommitmentFailure('booking', event.detail || {}, 'indeterminate', event.detail?.operation || event.detail?.kind || 'reserve'));
  document.addEventListener('nebula:consultation-notification-aborted', (event) => abortConsultationCommitment('notify', event.detail || {}));
  document.addEventListener('nebula:consultation-booking-aborted', (event) => abortConsultationCommitment('booking', event.detail || {}));
  document.addEventListener('nebula:consultation-notification-changed', (event) => updateConsultationCommitmentReceipt('notify', event.detail || {}, event.detail?.transition === 'cancelled' ? 'cancel' : 'notify'));
  document.addEventListener('nebula:consultation-booking-changed', (event) => updateConsultationCommitmentReceipt('booking', event.detail || {}, event.detail?.transition === 'cancelled' ? 'cancel' : 'reserve'));
  const closeConsultationConfirmation = ({ restoreFocus = true } = {}) => {
    if (!consultationConfirmation || consultationConfirmation.hidden) return;
    consultationConfirmation.hidden = true;
    consultationInertBackground.forEach((element) => { element.inert = false; });
    consultationInertBackground = [];
    consultationControls.forEach((control) => control.setAttribute('aria-expanded', 'false'));
    if (restoreFocus) consultationOpener?.focus();
    consultationOpener = null;
  };
  const openConsultationConfirmation = (opener, mode = 'start') => {
    if (!consultationConfirmation) return;
    closePopovers();
    consultationDialogMode = mode;
    consultationOpener = opener;
    if (consultationConfirmation.hidden) {
      // Keep conversation, expert and rate fixed while the modal owns focus.
      let branch = consultationConfirmation;
      while (branch.parentElement && branch !== document.body) {
        Array.from(branch.parentElement.children).forEach((sibling) => {
          if (sibling !== branch && !sibling.inert) {
            sibling.inert = true;
            consultationInertBackground.push(sibling);
          }
        });
    ...
    consultationConfirmation.dataset.mode = mode;
    consultationConfirmation.querySelector('#c76-consultation-confirmation-title').textContent = manage ? 'Pause or finish?' : ending ? 'End this consultation?' : resume ? 'Resume your consultation?' : 'Start your consultation?';
    consultationConfirmation.querySelector('[data-consultation-start-copy]').hidden = mode !== 'start';
    const copy = consultationConfirmation.querySelector('[data-consultation-mode-copy]');
    copy.hidden = mode === 'start';
    copy.textContent = manage ? 'You can request a pause or end this paid chat.' : ending ? 'You won’t be able to resume this consultation once it ends.' : 'Request to continue your paid chat at the rate below.';
    if (consultationDemo) {
      consultationConfirmation.querySelector('.c76-consultation-confirmation__eyebrow').textContent = 'DEMO CONSULTATION';
      consultationConfirmation.querySelector('.c76-consultation-confirmation__rate > div:last-child dd').textContent = 'Demo — no credits charged';
      if (mode === 'start') { copy.hidden = false; copy.textContent = ' Local demo: a 30-second connection preview opens first. The chat timer starts after connection; no credits are charged.'; }
    }
    consultationConfirmation.querySelector('.c76-consultation-confirmation__rate').hidden = manage || ending;
    const pauseOrEnd = consultationConfirmation.querySelector('[data-consultation-pause]');
    pauseOrEnd.hidden = !manage && !resume;
    pauseOrEnd.textContent = resume ? 'End consultation' : 'Request pause';
    pauseOrEnd.classList.toggle('is-secondary', resume);
    const confirm = consultationConfirmation.querySelector('[data-consultation-confirm]');
    confirm.parentElement.insertBefore(resume ? confirm : pauseOrEnd, resume ? pauseOrEnd : confirm);
    confirm.textContent = manage || ending ? 'End consultation' : resume ? 'Resume consultation' : 'Start consultation';
    confirm.classList.toggle('is-destructive', ending);
    confirm.classList.toggle('is-secondary', manage);
    consultationConfirmation.querySelector('.c76-consultation-confirmation__cancel').textContent = manage ? 'Keep chatting' : ending ? 'Go back' : resume ? 'Keep paused' : 'Not now';
    consultationConfirmation.hidden = false;
    consultationControls.forEach((control) => control.setAttribute('aria-expanded', 'true'));
    window.setTimeout(() => {
      if (!consultationConfirmation.hidden) consultationConfirmation.querySelector(manage ? '[data-consultation-pause]' : ending ? '.c76-consultation-confirmation__cancel' : '[data-consultation-confirm]')?.focus();
    }, 0);
  };
  const emitConsultationIntent = (intent) => {
    if (miaDialogueDemo && state.activeId === 'mia-jacomo') {
      document.dispatchEvent(new CustomEvent('nebula:mia-demo-intent', { detail: { action: intent === 'start' ? 'consent' : intent } }));
      return;
    }
    const allowed = { idle: ['start'], active: ['pause', 'end'], paused: ['resume', 'end'] };
    if (consultationPendingIntent || !allowed[consultationState]?.includes(intent)) return;
    if (consultationDemo && intent !== 'start') { applyDemoIntent(intent); return; }
    consultationPendingIntent = intent;
    if (consultationDemo) pendingDemoConnection = { requestId: `chat-demo-${++demoConnectionSequence}`, conversationId: state.activeId };
    renderConsultationControl();
    if (intent === 'start') {
      const expert = activeConversation();
      document.dispatchEvent(new CustomEvent('nebula:connection-presentation-open', { detail: {
        expertId: expert.id, expertName: expert.name, avatar: root.querySelector('[data-chat-avatar]')?.currentSrc || expert.avatar,
        statusAnchor: consultationControls[0], durationMs: 30000, presentationOnly: true,
        demo: consultationDemo, requestId: pendingDemoConnection?.requestId,
      } }));
    }
    if (consultationDemo) {
      if (document.querySelector('[data-nebula-connecting-expert]')?.hidden !== false) {
        pendingDemoConnection = null; consultationPendingIntent = null; renderConsultationControl();
        announce('Connection preview could not open. Please try again.');
      }
      return;
    }
    root.dispatchEvent(new CustomEvent(`nebula:consultation-${intent}-intent-required`, { bubbles: true, detail: { conversationId: state.activeId, backendRequired: true, sessionMutated: false, billingMutated: false, persisted: false, staticProjection: true, liveMount: false } }));
    announce('Awaiting consultation confirmation.');
  };
  let bookingOpenPending = false;
  const requestConsultationChange = (control) => {
    if (control.disabled || (consultationConfirmation && !consultationConfirmation.hidden)) return;
    const intent = control.dataset.consultationIntent || 'start';
    if (intent === 'notify' || intent === 'notify-cancel') {
      const expert = activeConversation();
      const miaNotification = miaDialogueDemo && expert.id === 'mia-jacomo';
      const detail = {
        expertId: expert.id, expertName: expert.name, opener: control, source: 'chatroom-consultation-control',
        mode: intent === 'notify-cancel' ? 'cancel' : 'subscribe',
        subscriptionId: intent === 'notify-cancel' ? (commitmentEntryFor(expert.id, 'notify')?.subscriptionId || null) : null,
        demo: miaNotification, backendRequired: !miaNotification, persisted: false,
      };
      if (miaNotification) document.dispatchEvent(new CustomEvent('nebula:consultation-notification-open', { detail }));
      else document.dispatchEvent(new CustomEvent('nebula:expert-card-action-open', { detail: { ...detail, kind: 'notify' } }));
      return;
    }
    if (!['book', 'book-cancel'].includes(intent) && miaDialogueDemo && state.activeId === 'mia-jacomo' && !['active', 'paused'].includes(consultationState)) {
      document.dispatchEvent(new CustomEvent('nebula:mia-demo-control', { detail: { opener: control } }));
      return;
    }
    if (intent === 'book' || intent === 'book-cancel') {
      if (bookingOpenPending) return;
      const expert = activeConversation();
      closePopovers();
      // Finish the originating swipe/click before the new scrim can receive its click.
      bookingOpenPending = true;
      window.setTimeout(() => {
        bookingOpenPending = false;
        if (!control.isConnected || control.disabled || activeConversation()?.id !== expert.id || control.dataset.consultationIntent !== intent) return;
        document.dispatchEvent(new CustomEvent('nebula:consultation-booking-open', { detail: {
          expertId: expert.id, expertName: expert.name, rate: expert.rate, opener: control,
          mode: intent === 'book-cancel' ? 'cancel' : 'choose',
        } }));
      }, 0);
      return;
    }
    openConsultationConfirmation(control, consultationState === 'active' ? 'manage' : consultationState === 'paused' ? 'resume' : 'start');
  };
  document.addEventListener('nebula:consultation-demo-booking-changed', (event) => {
    const detail = event.detail;
    if (detail?.demo !== true || detail.persisted !== false || !conversations.some((conversation) => conversation.id === detail.expertId)) return;
    const entry = commitmentEntryFor(detail.expertId, 'booking', detail.expertName || '');
    const request = entry.requestId ? { requestId: entry.requestId, idempotencyKey: entry.idempotencyKey } : {};
    updateConsultationCommitmentReceipt('booking', {
      ...detail, ...request, accepted: true, confirmed: true,
      transition: detail.booking ? 'confirmed' : 'cancelled',
      operation: detail.booking ? 'reserve' : 'cancel',
    }, detail.booking ? 'reserve' : 'cancel');
    renderConsultationControl();
  });
  document.addEventListener('nebula:consultation-booking-ready', () => renderConsultationControl());
  consultationControls.forEach((control) => {
    let pointerStart = null;
    let suppressPointerClick = false;
    control.addEventListener('click', (event) => {
      if (suppressPointerClick && event.detail !== 0) { suppressPointerClick = false; return; }
      requestConsultationChange(control);
    });
    if (control.getAttribute('role') !== 'switch') return;
    control.addEventListener('keydown', (event) => {
      if (!['ArrowLeft', 'ArrowRight'].includes(event.key)) return;
      event.preventDefault();
      const on = control.getAttribute('aria-checked') === 'true';
      if ((event.key === 'ArrowLeft' && on) || (event.key === 'ArrowRight' && !on)) requestConsultationChange(control);
    });
    control.addEventListener('pointerdown', (event) => {
      if (control.disabled || !event.isPrimary || event.button !== 0) return;
    ...
    control.addEventListener('pointerup', (event) => {
      ...
      const on = control.getAttribute('aria-checked') === 'true';
      if (Math.abs(dx) > Math.abs(dy) && ((dx < -10 && on) || (dx > 10 && !on))) requestConsultationChange(control);
    });
  });
  consultationConfirmation?.querySelectorAll('[data-consultation-cancel]').forEach((button) => button.addEventListener('click', () => {
    if (consultationDialogMode === 'end' && button.classList.contains('c76-consultation-confirmation__cancel')) openConsultationConfirmation(consultationOpener, consultationState === 'paused' ? 'resume' : 'manage');
    else closeConsultationConfirmation();
  }));
  consultationConfirmation?.querySelector('[data-consultation-confirm]')?.addEventListener('click', () => {
    if (consultationDialogMode === 'manage') { openConsultationConfirmation(consultationOpener, 'end'); return; }
    const intent = consultationDialogMode;
    closeConsultationConfirmation();
    emitConsultationIntent(intent);
  });
  consultationConfirmation?.querySelector('[data-consultation-pause]')?.addEventListener('click', () => {
    if (consultationDialogMode === 'resume') { openConsultationConfirmation(consultationOpener, 'end'); return; }
    closeConsultationConfirmation();
    emitConsultationIntent('pause');
  });
  root.addEventListener('nebula:consultation-host-snapshot', (event) => { if (!consultationDemo) applyConsultationHostSnapshot(event.detail); });
  // Explicit local demo adapter uses the same messages and consultation renderer.
  // It cannot mutate another expert or activate outside the server-marked test route.
  let miaRunId = null;
  let miaSessionId = null;
  let miaMessageSignature = '';
  document.addEventListener('nebula:mia-demo-snapshot', (event) => {
    const d = event.detail;
    if (!miaDialogueDemo || d?.demo !== true || d.expertId !== 'mia-jacomo') return;
    const conversation = conversations.find((item) => item.id === d.expertId);
    if (!conversation) return;
    const reset = miaRunId !== d.runId || miaSessionId !== d.sessionId;
    if (reset) {
      consultationByConversation.delete(d.expertId);
      consultationCommitmentsByConversation.delete(d.expertId);
      if (state.activeId === d.expertId) { consultationSeconds = 0; consultationState = 'idle'; consultationConfirmedState = 'idle'; consultationPendingIntent = null; }
      miaRunId = d.runId; miaSessionId = d.sessionId;
    }
    conversation.presence = d.presence === 'online' ? 'Online' : d.presence === 'busy' ? 'Busy' : 'Offline';
    conversation.online = d.presence === 'online';
    conversation.rate = '30 demo credits/min';
    const signature = JSON.stringify(d.messages);
    if (signature !== miaMessageSignature) {
      const previous = new Map(conversation.messages.map((m) => [m.id, m]));
      conversation.messages = d.messages.map((m) => make(m.role, m.text, { ...previous.get(m.id), id: m.id, from: m.role, text: m.text, time: new Date(m.at * 1000).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}), deliveryState: m.deliveryState, demo: true }));
      miaMessageSignature = signature;
      const rowPreview = conversation.row?.querySelector('.c76-thread__preview-row small');
      if (rowPreview) rowPreview.textContent = d.messages.at(-1)?.text || 'Free dialogue';
      state.defaultOpen = false;
      if (state.activeId === d.expertId) renderMessages({ keepScroll: true });
    }
    const phase = { active: 'active', paused: 'paused', disconnected: 'paused', completed: 'ended', awaiting_start: 'pending_start', connecting: 'pending_start' }[d.phase] || 'idle';
    applyConsultationHostSnapshot({role: 'client', conversationId: d.expertId, state: phase, elapsedSeconds: d.billedSeconds, minuteProgress: (d.billedSeconds % 60) / 60});
    if (state.activeId === d.expertId) {
      const presence = root.querySelector('[data-chat-presence]');
      if (presence) { presence.textContent = d.typing ? 'Mia is typing… (demo)' : conversation.presence; presence.dataset.presenceState = d.presence; }
      const rate = root.querySelector('[data-chat-rate]');
      if (rate) rate.textContent = conversation.rate;
      root.querySelectorAll('[data-consultation-rate]').forEach((n) => { n.textContent = conversation.rate; });
    }
  });
  document.addEventListener('nebula:mia-demo-message-accepted', (event) => {
    if (!miaDialogueDemo || state.activeId !== 'mia-jacomo') return;
    if (input.value.trim() === event.detail?.text) { input.value = ''; activeConversation().draft = ''; updateComposer(); }
  });
  document.addEventListener('nebula:mia-demo-offer-open', (event) => {
    if (!miaDialogueDemo || state.activeId !== 'mia-jacomo') return;
    openConsultationConfirmation(event.detail?.opener || consultationControls[0], 'start');
    consultationConfirmation.querySelector('#c76-consultation-confirmation-title').textContent = 'Accept Mia’s test offer?';
    consultationConfirmation.querySelector('.c76-consultation-confirmation__eyebrow').textContent = 'AUTOMATED LOCAL DEMO';
    const copy = consultationConfirmation.querySelector('[data-consultation-mode-copy]');
    copy.hidden = false;
    copy.textContent = '30 demo credits/min, charged by the second. Pauses are free. Accepting does not start billing: Mia starts the session separately. No real payment.';
    consultationConfirmation.querySelector('[data-consultation-start-copy]').hidden = true;
    consultationConfirmation.querySelector('.c76-consultation-confirmation__rate > div:last-child dd').textContent = 'Test balance only';
    consultationConfirmation.querySelector('[data-consultation-confirm]').textContent = 'Accept test offer';
  });
  document.addEventListener('nebula:mia-demo-offer-close', () => {
    if (miaDialogueDemo && consultationDialogMode === 'start') closeConsultationConfirmation();
  });
  const settleDemoConnection = (event) => {
    const pending = pendingDemoConnection;
    if (!consultationDemo || !pending || event.detail?.requestId !== pending.requestId || event.detail.expertId !== pending.conversationId) return;
    pendingDemoConnection = null;
    consultationPendingIntent = null;
    if (event.type === 'nebula:connection-demo-ready' && state.activeId === pending.conversationId) applyDemoIntent('start');
    else { renderConsultationControl(); announce('Connection cancelled. Consultation has not started.'); }
  };
  document.addEventListener('nebula:connection-demo-ready', settleDemoConnection);
  document.addEventListener('nebula:connection-demo-cancelled', settleDemoConnection);
  window.addEventListener('pagehide', () => { if (consultationDemoClock !== null) window.clearInterval(consultationDemoClock); consultationDemoClock = null; });
  window.addEventListener('pageshow', () => { if (consultationDemo) { renderDemoSessions(); syncDemoClock(); } });
  renderConsultationControl();

  const renderMediaViewer = () => {
    const media = mediaItems[mediaIndex];
    if (!media || !mediaStage) return;
    const hasMultiple = mediaItems.length > 1;
    mediaTitle.textContent = media.title || 'Chat media';
    const previous = hasMultiple ? '<button class="c76-media-viewer__nav c76-media-viewer__nav--previous" type="button" aria-label="Previous attachment" data-media-viewer-previous>‹</button>' : '';
    const next = hasMultiple ? '<button class="c76-media-viewer__nav c76-media-viewer__nav--next" type="button" aria-label="Next attachment" data-media-viewer-next>›</button>' : '';
    const visual = media.kind === 'video' && media.localObjectUrl
      ? `<div class="c76-video-demo c76-video-demo--native"><video src="${escapeHtml(media.src)}" controls preload="metadata" playsinline aria-label="${escapeHtml(media.title)}"></video><small>Local preview · not uploaded</small></div>`
      : media.kind === 'video'
        ? `<div class="c76-video-demo" data-video-demo><img src="${escapeHtml(media.src)}" alt="${escapeHtml(media.title)}"><button type="button" data-video-unavailable aria-label="Video source unavailable" disabled>${playAssetMarkup('c76-video-demo__play')}<span>Video unavailable</span></button><div aria-hidden="true"><i data-video-progress></i></div><small>Source poster only · playable video not provided</small></div>`
        : `<img src="${escapeHtml(media.src)}" alt="${escapeHtml(media.title)}">`;
    mediaStage.innerHTML = `${previous}${visual}${next}`;
    if (mediaThumbnails) mediaThumbnails.innerHTML = mediaItems.map((item, index) => `<button type="button" class="c76-media-viewer__thumbnail${index === mediaIndex ? ' is-current' : ''}" data-media-viewer-thumbnail="${index}" aria-label="Open ${escapeHtml(item.title || `attachment ${index + 1}`)}" aria-current="${index === mediaIndex ? 'true' : 'false'}"><img src="${escapeHtml(item.src)}" alt=""><span>${item.kind === 'video' ? 'Video' : 'Photo'}</span></button>`).join('');
  };
  const moveMedia = (step) => {
    if (mediaItems.length < 2) return;
    mediaIndex = (mediaIndex + step + mediaItems.length) % mediaItems.length;
    renderMediaViewer();
    mediaViewer.querySelector(`[data-media-viewer-thumbnail="${mediaIndex}"]`)?.focus();
  };
  function openMedia(items, index, opener) {
    mediaItems = Array.isArray(items) ? items.filter(Boolean) : [];
    mediaIndex = Math.min(Math.max(0, Number(index) || 0), Math.max(0, mediaItems.length - 1));
    if (!mediaItems.length) return;
    mediaOpener = opener; closePopovers(); mediaViewer.hidden = false; frame.inert = true; frame.setAttribute('inert', ''); frame.setAttribute('aria-hidden', 'true');
    renderMediaViewer();
    mediaViewer.querySelector('.c76-media-viewer__dialog [data-media-viewer-close]')?.focus();
  }
  const closeMedia = () => { if (mediaViewer.hidden) return; mediaViewer.hidden = true; frame.inert = false; frame.removeAttribute('inert'); frame.removeAttribute('aria-hidden'); mediaStage.replaceChildren(); mediaThumbnails?.replaceChildren(); mediaItems = []; mediaIndex = 0; mediaOpener?.focus(); mediaOpener = null; };
  ...
  mediaStage?.addEventListener('click', (event) => {
    const nav = event.target.closest('[data-media-viewer-previous],[data-media-viewer-next]');
    if (nav) moveMedia(nav.hasAttribute('data-media-viewer-next') ? 1 : -1);
  });
  mediaThumbnails?.addEventListener('click', (event) => { const button = event.target.closest('[data-media-viewer-thumbnail]'); if (!button) return; mediaIndex = Number(button.dataset.mediaViewerThumbnail) || 0; renderMediaViewer(); mediaViewer.querySelector(`[data-media-viewer-thumbnail="${mediaIndex}"]`)?.focus(); });

  document.addEventListener('keydown', (event) => {
    if (event.target.closest?.('.c76-header')) return;
    if (consultationConfirmation && !consultationConfirmation.hidden) {
      if (event.key === 'Escape') { event.preventDefault(); closeConsultationConfirmation(); return; }
      if (event.key === 'Tab') {
        const focusables = Array.from(consultationConfirmation.querySelectorAll('button:not([disabled])')).filter((item) => item.tabIndex >= 0 && item.getClientRects().length > 0);
    ...
    if (mediaViewer && !mediaViewer.hidden && (event.key === 'ArrowLeft' || event.key === 'ArrowRight') && !event.target.closest?.('video,audio')) { event.preventDefault(); moveMedia(event.key === 'ArrowRight' ? 1 : -1); return; }
    ...
    if (voiceRecorder && !voiceRecorder.hidden) { event.preventDefault(); cancelVoiceRecording(); return; }
  ...
  document.addEventListener('click', (event) => {
    ...
    const openPopover = root.querySelector('.c76-chat-search:not([hidden]),.c76-chat-more-menu:not([hidden]),.c76-message-menu:not([hidden]),.c76-reaction-picker,.c76-emoji-picker:not([hidden]),.c76-attachment-menu:not([hidden]),.c76-filter-menu:not([hidden])');
    if (openPopover && !openPopover.contains(event.target) && !popoverOpener?.contains(event.target)) closePopovers();
  });
  compact.addEventListener?.('change', () => { menu?.classList.remove('is-expanded'); syncCompactView(); syncConversationSearch(); syncConversationScrollbar(); });
  ...
  const syncUsableViewport = () => {
    const viewport = window.visualViewport;
    // Pinch zoom retains normal browser panning; only the unzoomed viewport sizes the app.
    if (viewport && viewport.scale !== 1) return;
    const height = Math.round(viewport?.height || window.innerHeight);
    root.style.setProperty('--c76-viewport-height', `${height}px`);
    root.dataset.compactViewport = String(height <= 550);
    root.style.setProperty('--c76-viewport-top', `${Math.round(viewport?.offsetTop || 0)}px`);
  };
  ...
  syncUsableViewport();
  renderSaved(); renderList(); renderConversation({ focus: false }); syncCompactView(); syncConversationSearch(); syncSuggestionNav(); syncScrollBottom();

  // Frontend route entry only. Resolve against the rendered conversation set;
  // never treat a URL value as a selector, expert identity, draft or send intent.
  // A controller-validated expert selection always takes precedence.
  const entry = new URLSearchParams(window.location.search);
  const entryValue = name => {
    const values = entry.getAll(name);
    return values.length === 1 ? values[0] : null;
  };
  if (!root.dataset.selectedExpertId) {
    const conversation = conversations.find(item => item.id === entryValue('conversation'));
    if (conversation) {
      selectConversation(conversation, conversation.row, false);
      const panel = entryValue('panel');
      const suggestion = entryValue('suggestion');
      if (panel === 'compose') {
        if (suggestion !== null && /^(0|[1-9]\d*)$/.test(suggestion)) {
          const chip = suggestions[Number(suggestion)];
          if (chip) { conversation.draft = chip.textContent.trim(); input.value = conversation.draft; updateComposer(); }
        }
        input?.focus();
      } else if (['attachment', 'emoji'].includes(panel)) {
        root.querySelector(`[data-composer-action="${panel}"]`)?.click();
      } else if (panel === 'voice') {
        // Entering a route never starts recording, even the local preview.
        root.querySelector('[data-composer-action="voice"]')?.focus();
      } else if (panel === 'favorite') {
        actionButton('favorite')?.focus();
      } else if (['search', 'more'].includes(panel)) {
        actionButton(panel)?.click();
      } else {
        const heading = threadPanel?.querySelector('h1');
        if (heading) { heading.tabIndex = -1; heading.focus(); }
      }
    } else if (!entry.has('conversation') && entryValue('view') === 'saved') {
      openSavedView();
    }
  }
})();
