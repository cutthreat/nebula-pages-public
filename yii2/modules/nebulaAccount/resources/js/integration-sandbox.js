(() => {
  ...
  if (window.NebulaIntegration) return;
  const carrier = document.querySelector('meta[name="nebula-integration-config"]');
  if (!carrier) return;
  let settings;
  try { settings = JSON.parse(carrier.content); } catch { return; }
  if (!settings || Array.isArray(settings) || settings.mode !== 'local-file-backed-v1' || typeof settings.baseUrl !== 'string') return;
  const config = Object.freeze({
    ...settings,
    csrfToken: document.querySelector('meta[name="csrf-token"]')?.content || '',
  });
  window.NebulaIntegrationConfig = config;
  ...
  const syncBalance = (state) => {
    ...
    document.querySelectorAll('.nb-credit-pill, [data-credit-topup-trigger]').forEach((node) => { node.setAttribute('aria-label', `Credits balance ${value}`); });
    document.querySelectorAll('.nb-sidebar-balance strong span, .hb-balance strong span').forEach((node) => { node.textContent = `${value} credits`; });
    document.querySelectorAll('.nb-affirmation-balance strong').forEach((node) => { node.textContent = `Balance: ${value} credits`; });
  ...
  const persistIntent = async (event, mapping) => {
    // Source previews declare no persistence/session mutation: do not turn their
    // host-required intent into a sandbox write merely because the adapter is mounted.
    if (!event?.detail || event.detail.integrationHandled === true || event.detail.staticProjection === true) return;
  ...
  Object.entries(workflowEvents).forEach(([eventName, descriptor]) => {
    document.addEventListener(eventName, async (event) => {
      if (event?.detail?.integrationHandled === true || event?.detail?.staticProjection === true) return;
