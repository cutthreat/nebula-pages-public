(() => {
  ...
  const setChecked = (option, active) => {
    ...
    option.querySelector('img').src = option.querySelector('img').src.replace(/(?:un)?selected\.svg(?=[?#]|$)/, active ? 'selected.svg' : 'unselected.svg');
  };
  const selectedChannels = () => options.filter((option) => option.getAttribute('aria-checked') === 'true').map((option) => option.dataset.channel);
  const detail = (reason) => ({ reason, notificationIntentId: root.dataset.flowId || null, version: root.dataset.flowVersion || null, subjectId: root.dataset.subjectId || null, selectedChannelIds: selectedChannels(), preferenceSaved: false, subscriptionCreated: false, notificationSent: false, deliveryClaimed: false });
  ...
  root.addEventListener('nebula:notification-destination-open', (event) => {
    const payload = event.detail || {};
    const expectedIds = ['email', 'sms', 'telegram'];
    const expectedLabels = ['Email', 'SMS (text message)', 'Telegram'];
    const selectedIds = Array.isArray(payload.selectedChannelIds) ? Array.from(payload.selectedChannelIds, String) : [];
    // Match the existing PHP snapshot contract before relabeling this mounted flow.
    const channelsValid = Array.isArray(payload.availableChannels) && payload.availableChannels.length === 3
      && expectedIds.every((id, index) => {
        const channel = payload.availableChannels[index];
        return channel?.id === id && channel.label === expectedLabels[index] && channel.available === true
          && payload.verifiedEndpoints?.[id] === true && payload.consent?.[id] === true;
      });
    const valid = payload.serverState === 'notification_channel_selection' && (payload.notificationIntentId || payload.notificationIntentId === 0) && payload.version != null
      && (payload.subjectId || payload.subjectId === 0) && payload.authenticatedClient === true && channelsValid
      && payload.capabilities?.notificationDestinationSelection === true
      && selectedIds.every((id) => expectedIds.includes(id));
    if (!valid) { root.dataset.lastAction = 'open-rejected-authoritative-notification-flow-required'; return; }
    opener = payload.opener instanceof HTMLElement ? payload.opener : opener;
    root.dataset.flowId = String(payload.notificationIntentId);
    if (payload.version != null) root.dataset.flowVersion = String(payload.version);
    root.dataset.subjectId = String(payload.subjectId);
    options.forEach((option) => setChecked(option, selectedIds.includes(option.dataset.channel)));
