(() => {
  ...
  const render = () => {
    ...
    // The primary action is the single reconciliation affordance; keep the
    // implementation hook in the DOM for host adapters without duplicating
    // two identical buttons in the product surface.
    ...
    if (!modal.hidden) window.setTimeout(() => (indeterminate ? submit : (pending ? secondary : heading))?.focus(), 0);
  };
  ...
  const beginOperation = (kind, { reconcileRequest = false } = {}) => {
    ...
    if (isCancel) emit('nebula:consultation-notification-cancel-intent', detail);
  ...
  document.addEventListener('nebula:mia-demo-reset', (event) => {
    ...
    clearTimers();
    operation = null;
    state.phase = 'idle'; state.confirmed = false; state.operation = null; state.requestId = null; state.idempotencyKey = null; state.errorMessage = ''; state.updatedAt = now();
    publish(state);
