(() => {
  ...
  if (!root) return;
  const paidSourcePreview = root.dataset.sourceVariant === 'paid_active';
  ...
  const consultationControl = root.querySelector('[data-active-action="consultation-control"]');
  const consultationTimer = root.querySelector('[data-active-timer]');
  const activePinBanner = root.querySelector('[data-active-pin-banner]');
  const activePinDivider = root.querySelector('[data-active-pin-divider]');
  ...
  const activePinMenuIcon = activePinBanner?.querySelector('.active-pin-icon')?.getAttribute('src') || '';
  const activeMenuItems = [
    ...
    `<button type="button" role="menuitem" data-active-chat-menu="pin">${activePinMenuIcon ? `<img class="active-chat-menu__icon" src="${activePinMenuIcon}" alt="" aria-hidden="true">` : ''}<span data-active-pin-menu-label>Pin message</span></button>`,
  ...
  activeMoreMenu?.addEventListener('click', (event) => {
    ...
    if (action === 'mute') {
      ...
      renderActivePin();
      intent('nebula:chat-message-pin-intent', { conversationId: 'miss-shaya', messageId: 'active-greeting', pinned: activePinned, backendRequired: true, persisted: false, staticProjection: true, liveMount: false });
      announce(activePinned ? 'Message pinned locally. Server confirmation required.' : 'Message unpinned locally. Server confirmation required.');
    } else {
      const eventMap = {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (!activeMoreMenu.contains(event.target) && !activeMore.contains(event.target)) closeActiveMore();
  }, true);
  ...
  let activePinned = paidSourcePreview;
  const announce = (message) => { if (live) live.textContent = message; };
  const renderActivePin = () => {
    if (activePinBanner) activePinBanner.hidden = !activePinned;
    if (activePinDivider) activePinDivider.hidden = !activePinned;
    const label = activeMoreMenu?.querySelector('[data-active-pin-menu-label]');
    if (label) label.textContent = activePinned ? 'Unpin message' : 'Pin message';
  };
  ...
  const stopConsultationTimer = () => {
    if (!consultationTimerId) return;
    window.clearInterval(consultationTimerId);
    consultationTimerId = null;
  };
  const startConsultationTimer = () => {
    if (paidSourcePreview) return;
    if (consultationTimerId) return;
    consultationTimerId = window.setInterval(() => {
      consultationSeconds += 1;
      if (consultationTimer) consultationTimer.textContent = formatConsultationTime(consultationSeconds);
    }, 1000);
  };
  const renderConsultation = (state, source = 'fixture') => {
    if (!consultationControl || !['running', 'stopped', 'paused'].includes(state)) return;
    const copy = {
      running: { label: 'Stop chat', aria: 'Stop paid chat' },
      stopped: { label: 'Start chat', aria: 'Start paid chat' },
      paused: { label: 'Paused', aria: 'Paid chat paused by expert' },
    }[state];
    consultationControl.dataset.consultationState = state;
    consultationControl.disabled = state === 'paused';
    consultationControl.setAttribute('aria-disabled', state === 'paused' ? 'true' : 'false');
    consultationControl.setAttribute('aria-label', copy.aria);
    const label = consultationControl.querySelector('[data-consultation-label]');
    if (label) label.firstChild.nodeValue = copy.label;
    if (consultationTimer) consultationTimer.textContent = formatConsultationTime(consultationSeconds);
    root.dataset.consultationState = state;
    root.dataset.consultationStateSource = source;
    if (state === 'running') startConsultationTimer(); else stopConsultationTimer();
  };
  const syncComposer = () => {
    if (!composer || !send) return;
    if (paidSourcePreview) {
      composer.rows = 1;
    ...
    const multiline = composer.value.includes('\n') || (paidSourcePreview
      ? composer.scrollHeight > parseFloat(window.getComputedStyle(composer).lineHeight) + 2
    ...
    const assetState = paidSourcePreview && ready ? 'ready' : activeState;
    const stateSrc = sendStateImage?.dataset[`src${assetState[0].toUpperCase()}${assetState.slice(1)}`];
  ...
  if (paidSourcePreview) {
    ...
    document.fonts?.ready.then(syncComposer);
  }
  consultationControl?.addEventListener('click', () => {
    if (paidSourcePreview) {
      intent('nebula:end-chat-intent-required', { backendRequired: true, sessionMutated: false, timerMutated: false, billingMutated: false, persisted: false, staticProjection: true, liveMount: false });
      announce('Ending the paid chat requires server confirmation. The source preview and balance are unchanged.');
      return;
    }
    const running = consultationControl.dataset.consultationState === 'running';
    const next = running ? 'stopped' : 'running';
    if (!running) consultationSeconds = 0;
    renderConsultation(next, 'client-fixture');
    intent(running ? 'nebula:end-chat-intent-required' : 'nebula:consultation-start-intent-required', {
      backendRequired: true,
      demoOnly: true,
    ...
    announce(next === 'running' ? 'Paid chat started locally. Server confirmation and billing are not simulated.' : 'Paid chat stopped locally. No server session or billing state was changed.');
  });
  root.addEventListener('nebula:consultation-expert-state', (event) => {
    if (paidSourcePreview) return;
    const state = event.detail?.state;
    if (!['running', 'paused'].includes(state)) return;
    renderConsultation(state, 'expert-adapter');
    announce(state === 'paused' ? 'Paid chat paused by expert. Ask the expert in chat to resume.' : 'Paid chat resumed by expert locally.');
  });
  if (paidSourcePreview) {
    root.dataset.consultationState = 'running';
    root.dataset.consultationStateSource = 'figma-static-sample';
  } else {
    renderConsultation(consultationControl?.dataset.consultationState || 'running', 'initial-fixture');
    if (new URLSearchParams(window.location.search).get('clientFlow') === 'psychics-start') {
      root.dataset.clientFlow = 'psychics-start';
      intent('nebula:chat-client-preview-active', {
        backendRequired: true,
      ...
      announce('Paid-chat client preview is active. Server confirmation and billing are pending.');
    }
  }
  renderActivePin();
  ...
  const activeBack = root.querySelector('.active-back');
  if (paidSourcePreview && activeBack) {
    activeBack.setAttribute('aria-controls', 'active-list-drawer');
    activeBack.setAttribute('aria-expanded', 'false');
  }
  activeBack?.addEventListener('click', () => {
    if (paidSourcePreview) { setDrawer(true); return; }
  ...
  const setDrawer = (open) => {
    ...
    if (paidSourcePreview) activeBack?.setAttribute('aria-expanded', open ? 'true' : 'false');
    if (scrim) scrim.hidden = !open;
    if (open) {
      drawerOrigin = paidSourcePreview ? activeBack : (document.activeElement === rail ? rail : (drawerOrigin || rail));
      drawerClose?.focus();
    } else {
      drawerOrigin?.focus();
      drawerOrigin = null;
