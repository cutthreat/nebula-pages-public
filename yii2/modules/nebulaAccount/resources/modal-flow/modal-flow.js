...
(function () {
  ...
  function focusScenario() {
    ...
    if (selectedState === 'video' && scenarioRoot.matches('.video-preview-static[role="dialog"]')) {
      scenarioRoot.focus({preventScroll: true});
      return;
    }
  ...
  function activeDialog() {
    ...
    if (selectedState === 'video' && scenarioRoot.matches('.video-preview-static[role="dialog"]')) return scenarioRoot;
  ...
  function focusableElements(dialog) {
    ...
    return Array.prototype.filter.call(dialog.querySelectorAll(
      ...
      return !element.hidden && element.getClientRects().length > 0 && element.getAttribute('aria-hidden') !== 'true' && element.getAttribute('tabindex') !== '-1';
    });
  }
  ...
  function isDismissControl(target) {
    return Boolean(target.closest(
      ...
      '.empty__close,.endstay__close,.delivery__close,.offline__close,' +
      '.astrological-details [data-action="dismiss"],.astrological-details [data-action="back"]'
    ));
  }
  ...
  registry.addEventListener('click', function (event) {
    if (!scenarioRoot || scenarioRoot.hidden || !scenarioRoot.contains(event.target)) return;
    // The unavailable partial is a sibling of Chatroom, so its fixture owns these bindings.
    if (selectedState === 'unavailable' && scenarioRoot.matches('[data-expert-unavailable-modal]')) {
      if (!event.target.getClientRects().length) return;
      var control = event.target.closest('[data-unavailable-action]');
      if (control && control.disabled) return;
      var action = control && control.dataset.unavailableAction;
      var backdrop = scenarioRoot.querySelector('[data-action="close-expert-unavailable"]');
      if (event.target === backdrop || action === 'wait') {
        unlockBackground();
        return;
      }
      if (action === 'select-another' || action === 'question-in-advance') {
        var selecting = action === 'select-another';
        scenarioRoot.dataset.lastAction = selecting ? 'select-another-expert-server-required' : 'question-in-advance-state-required';
        scenarioRoot.dispatchEvent(new CustomEvent(selecting ? 'nebula:select-alternative-expert' : 'nebula:open-question-before-start', {bubbles: true}));
        announce(selecting ? 'Finding an eligible available expert requires the matching service.' : 'The advance question form is not connected yet.');
        return;
      }
    }
  ...
  document.addEventListener('keydown', function (event) {
    ...
    if (event.key === 'Tab') {
      ...
      if (dialog === scenarioRoot && selectedState === 'video' && !focusables.length) {
