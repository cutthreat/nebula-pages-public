(() => {
  const root = document.querySelector('[data-nebula-special-offers]');
  if (!root) return;
  const status = root.querySelector('[data-notification-status]');
  const sync = (control, enabled) => {
    control.setAttribute('aria-pressed', String(enabled));
    const label = control.dataset.notificationToggle === 'offers' ? 'Allow notification from psychics' : control.dataset.notificationToggle;
    control.setAttribute('aria-label', `${label}: ${enabled ? 'on' : 'off'}`);
