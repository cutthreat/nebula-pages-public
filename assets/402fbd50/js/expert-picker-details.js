(() => {
  ...
  if (!root) return;
  const mountedExpertId = root.dataset.expertId;
  if (!mountedExpertId) return;
  ...
  const goBack = () => {
    ...
    emit('nebula:expert-details-generic-picker-return-required', { selectedExpertId: mountedExpertId });
    pickerOpener?.focus();
  ...
  favorite.addEventListener('click', () => {
    ...
    emit('nebula:expert-favorite-intent-required', { expertId: mountedExpertId, localSelected: selected, persisted: false });
  ...
  reviews.addEventListener('click', () => {
    ...
    emit('nebula:expert-reviews-intent-required', { expertId: mountedExpertId });
    announce('Reviews destination requires a host adapter.');
  });
  payment.addEventListener('click', () => {
    root.dataset.lastAction = 'payment-preflight-required';
    emit('nebula:payment-preflight-required', { expertId: mountedExpertId });
  ...
  root.addEventListener('nebula:expert-details-before-selection-open', (event) => {
    // This source-bound view is a fixed profile, not a dynamic profile renderer.
    if (event.detail?.expertId != null && String(event.detail.expertId) !== mountedExpertId) {
      root.dataset.lastAction = 'open-rejected-mounted-expert-mismatch';
      announce('The requested expert requires a matching profile source.');
      return;
    }
