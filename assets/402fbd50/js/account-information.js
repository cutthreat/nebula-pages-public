(() => {
  ...
  document.addEventListener('nebula:integration-receipt', (event) => {
    ...
    if (detail.command !== 'profile.field.update') return;
    const row = root.querySelector(`[data-account-information-row][data-field="${CSS.escape(detail.payload?.field || '')}"]`);
    if (!row) return;
  ...
  document.addEventListener('nebula:integration-error', (event) => {
    ...
    if (display) display.textContent = row.dataset.field === 'password' ? '• • • • • • • •' : previous;
    row.dataset.integrationPending = 'false';
    row.dataset.integrationPersisted = 'false';
    row.querySelector('[data-account-information-edit]')?.removeAttribute('hidden');
    setInvalid(row, detail.message || 'The integration host rejected this value.');
  });
})();
