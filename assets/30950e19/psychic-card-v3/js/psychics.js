(() => {
  ...
  const applyFilters = (options = {}) => {
    ...
    if (filterStatus) filterStatus.textContent = `${visible} experts shown locally. Saving filters requires the host.`;
    emit('nebula:psychic-filter-intent-required', { backendRequired: true, persisted: false, staticProjection: true, filters: { selectedStatus, specialties, minExperience, minRating, sort } });
  ...
  const setCollectionExpanded = (section, expanded, trigger, options = {}) => {
    ...
    if (focus) section.querySelector('[data-psychic-card]:not([hidden])')?.focus?.({ preventScroll: true });
  ...
  unavailable?.querySelector('[data-unavailable-action="question-in-advance"]')?.addEventListener('click', () => {
    ...
    questionBeforeStart?.dispatchEvent(new CustomEvent('nebula:question-before-start-open', {
      bubbles: true,
      detail: { trigger: unavailableReturnFocus, selectedExpertId: 'mia-jacomo', staticProjection: true },
    }));
  });
  ...
  trialPicker?.addEventListener('nebula:trial-selection-commit-required', (event) => {
    const selectedExpertId = event.detail?.selectedExpertId || 'margo-lover';
    ...
    questionBeforeStart?.dispatchEvent(new CustomEvent('nebula:question-before-start-open', {
      bubbles: true,
      detail: { trigger: root.querySelector('[data-psychic-gift]'), selectedExpertId, staticProjection: true },
    }));
  });

  // Reuse the C76 checkout panels between a question and the existing
  // connection state. The approved event is fixture-only: it creates neither
  // a payment nor a balance mutation, and maps directly to provider success
  // when the host later supplies one.
  document.addEventListener('nebula:question-before-start-submit-required', (event) => {
    if (event.target !== questionBeforeStart) return;
    const questionFrame = questionBeforeStart.querySelector('.question-before-start__frame');
    questionFrame && (questionFrame.hidden = true);
    questionBeforeStart.hidden = true;
    questionBeforeStart.dataset.lastAction = 'question-context-accepted-open-checkout';
    // Release the question dialog's lock before the next C76 dialog takes it.
    // Otherwise the checkout is visible but inherits a stale inert snapshot.
    root.removeAttribute('inert');
    root.removeAttribute('aria-hidden');
    document.documentElement.classList.remove('nebula-modal-open');
    const creditTopup = document.querySelector('[data-nebula-credit-topup]');
    if (creditTopup) {
      creditTopup.dispatchEvent(new CustomEvent('nebula:credit-topup-open-requested', {
        bubbles: true,
        detail: { opener: event.detail?.trigger instanceof HTMLElement ? event.detail.trigger : root.querySelector('[data-psychic-start]') },
      }));
      return;
    }
    if (!connectingExpert) return;
    connectingExpert.dispatchEvent(new CustomEvent('nebula:consultation-connecting-open', {
      bubbles: true,
      detail: {
        serverState: 'connecting',
        sessionId: 'sandbox-psychics-connect',
        statusAnchor: event.detail?.trigger instanceof HTMLElement
          ? event.detail.trigger
          : root.querySelector('[data-psychic-start]'),
        expertSnapshot: { displayName: 'Margo Lover' },
        fixture: true,
        persisted: false,
        sessionCreated: false,
      },
  ...
  document.addEventListener('nebula:credit-topup-payment-approved', (event) => {
    if (event.detail?.context !== 'psychics-paid-chat' || !connectingExpert) return;
    connectingExpert.dispatchEvent(new CustomEvent('nebula:consultation-connecting-open', {
      bubbles: true,
      detail: {
        serverState: 'connecting',
        sessionId: 'sandbox-psychics-paid-chat',
        statusAnchor: event.detail?.opener instanceof HTMLElement ? event.detail.opener : root.querySelector('[data-psychic-start]'),
        expertSnapshot: { displayName: 'Margo Lover' },
        fixture: true,
  ...
  root.addEventListener('click', (event) => {
    ...
    if (start && root.contains(start)) {
      ...
      questionBeforeStart?.dispatchEvent(new CustomEvent('nebula:question-before-start-open', {
        ...
        detail: {
          trigger: start,
          selectedExpertId: start.closest('[data-psychic-card]')?.dataset.profileUrl || '',
          staticProjection: true,
        },
      }));
      return;
    ...
    if (pageButton && root.contains(pageButton)) {
      ...
      section.querySelector('[data-psychic-card]:not([hidden])')?.focus?.({ preventScroll: true });
      return;
    }
  ...
  root.addEventListener('keydown', (event) => {
    ...
    const card = event.target.closest('[data-psychic-card][data-profile-url]');
