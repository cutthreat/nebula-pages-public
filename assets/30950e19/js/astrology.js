(() => {
  ...
  const confirmationDuration = 2800;
  const specs = {
    date: {
      validate: (value) => {
  ...
  const finishEdit = (row, result) => {
    ...
    row.dataset.integrationPreviousValue = row.dataset.value || '';
    row.dataset.integrationPending = window.NebulaIntegration ? 'true' : 'false';
  ...
  const startEdit = (row) => {
    ...
    const edit = row.querySelector('[data-astrology-edit]');
    if (!input || !display || !edit) return;
    // Keep the editor deterministic: one unfinished change at a time.  This
    // mirrors the account form and prevents an unsaved value from being left
    // in a second row when the user changes their mind.
    root.querySelectorAll('[data-astrology-row].is-editing').forEach((activeRow) => {
      if (activeRow !== row) cancel(activeRow);
    });
    clearCheckTimer(row);
    setCheck(row, false);
    clearInvalid(row);
  ...
  document.addEventListener('nebula:integration-receipt', (event) => {
    ...
    if (detail.command !== 'astrology.field.update') return;
    const row = root.querySelector(`[data-astrology-row][data-field="${CSS.escape(detail.payload?.field || '')}"]`);
    if (!row) return;
    row.dataset.integrationPending = 'false';
    row.dataset.integrationPersisted = 'true';
    delete row.dataset.integrationPreviousValue;
    announce(`${row.querySelector('strong')?.textContent || 'Field'} saved in the local integration repository.`);
  });
  document.addEventListener('nebula:integration-error', (event) => {
    ...
    const row = root.querySelector(`[data-astrology-row][data-field="${CSS.escape(detail.payload?.field || '')}"]`);
    if (!row) return;
    const previous = row.dataset.integrationPreviousValue ?? row.dataset.value ?? '';
    const display = row.querySelector('[data-astrology-display]');
    row.dataset.value = previous;
    if (display) display.textContent = previous;
    row.dataset.integrationPending = 'false';
    row.dataset.integrationPersisted = 'false';
    row.querySelector('[data-astrology-edit]')?.removeAttribute('hidden');
    setInvalid(row, { key: detail.payload?.field || '', value: detail.payload?.value || '', message: detail.message || 'The integration host rejected this value.' });
  });
})();
