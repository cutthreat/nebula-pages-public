(() => {
  ...
  const voiceCaption = root.querySelector('[data-voice-caption]');
  ...
  messagesNode?.addEventListener('click', async (event) => {
    ...
    if (control.dataset.reaction && message) {
      ...
      root.dispatchEvent(new CustomEvent('nebula:chat-message-reaction-intent', { bubbles: true, detail: { conversationId: conversation.id, messageId: message.id, reaction: selected ? control.dataset.reaction : '', selected, persisted: false, localProjection: true } }));
      renderMessages({ keepScroll: true });
  ...
  const updateVoiceRecorder = () => {
    ...
    if (voiceRecorder) voiceRecorder.dataset.state = voiceTimer ? 'recording' : 'ready';
    if (voiceCaption) voiceCaption.textContent = voiceSeconds >= 60 ? 'preview ready' : 'local preview';
  };
  ...
  const runVoiceTimer = () => {
    ...
    voiceTimer = window.setInterval(() => {
      ...
      if (voiceSeconds >= 60) {
        clearVoiceTimer();
        updateVoiceRecorder();
        announce('Local voice preview reached one minute. Send or discard it.');
      }
    }, 1000);
  };
  ...
  const startVoiceRecording = () => {
    ...
    announce('Local voice preview started. A waveform is shown; no microphone is used. Use discard or send.');
  };
  ...
  const renderConsultationControl = () => {
    ...
    consultationControls.forEach((control) => {
      control.dataset.consultationState = 'idle';
      control.setAttribute('aria-pressed', 'false');
      const label = control.querySelector('[data-consultation-label]');
      if (label) label.textContent = control.classList.contains('c76-consultation-action') ? 'Start' : 'Start consultation';
      control.setAttribute('aria-label', 'Start consultation');
    });
  };
  consultationControls.forEach((control) => control.addEventListener('click', () => {
    root.dispatchEvent(new CustomEvent('nebula:consultation-start-intent-required', { bubbles: true, detail: { backendRequired: true, sessionMutated: false, timerStarted: false, timerMutated: false, billingMutated: false, persisted: false, staticProjection: true, liveMount: false } }));
    announce('Consultation start intent sent to the host. The page will change only after an authoritative host outcome.');
    showToast('Waiting for host confirmation');
  }));
  ...
  document.addEventListener('keydown', (event) => {
    if (mediaViewer && !mediaViewer.hidden && (event.key === 'ArrowLeft' || event.key === 'ArrowRight')) { event.preventDefault(); moveMedia(event.key === 'ArrowRight' ? 1 : -1); return; }
