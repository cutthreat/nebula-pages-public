(() => {
  ...
  const conversations = rows
    ...
    .map((row) => {
      ...
      return {
        ...
        online: rowProfiles[name]?.online || seed.presence === 'Online', muted: rowProfiles[name]?.muted || false,
  ...
  const renderAttachment = () => {
    ...
    preview.className = 'c76-attachment-preview';
    const thumb = document.createElement('span');
    thumb.className = 'c76-attachment-preview__thumb';
    thumb.setAttribute('aria-hidden', 'true');
    if (attachment.kind === 'photo' && attachment.src) {
      ...
      thumb.textContent = attachment.kind === 'video' ? '▶' : attachment.kind === 'voice' ? '🎙' : '📎';
    }
    const info = document.createElement('span'); info.className = 'c76-attachment-preview__info';
    const title = document.createElement('b'); title.textContent = attachment.title || 'Selected file';
    const meta = document.createElement('small'); meta.textContent = `${attachment.kind === 'photo' ? 'Photo' : attachment.kind === 'video' ? 'Video' : attachment.kind === 'voice' ? 'Voice preview' : 'Document'} · ${formatBytes(attachment.size)} · local preview`;
  ...
  const mediaMarkup = (media) => {
    ...
    if (media.kind === 'video') {
      ...
      return `<button class="c76-media-card c76-media-card--video" type="button" data-media-open="video" aria-label="Preview ${escapeHtml(media.title)}">${visual}<span class="c76-media-card__play">▶</span><small>${escapeHtml(media.duration || 'Video')} · ${media.localObjectUrl ? 'local preview' : 'source preview'}</small></button>`;
    }
  ...
  const renderMessages = ({ keepScroll = false } = {}) => {
    ...
    messagesNode.innerHTML = conversation.messages.map((message) => {
      ...
      const voice = message.voice ? `<div class="c76-voice-card"><button type="button" data-voice-play aria-label="Play local voice preview" aria-pressed="false">▶</button><span class="c76-voice-wave" aria-hidden="true"></span><small>${escapeHtml(message.voice.duration)}</small></div>` : '';
      const reactions = Object.entries(message.reactions || {}).map(([emoji, count]) => `<button type="button" data-reaction="${escapeHtml(emoji)}" aria-label="${escapeHtml(emoji)} reaction, ${count}">${escapeHtml(emoji)} <b>${count}</b></button>`).join('');
  ...
  const renderSaved = () => {
    ...
    if (!state.saved.length) {
      const empty = document.createElement('p'); empty.className = 'c76-saved-empty'; empty.dataset.savedEmpty = ''; empty.textContent = 'No saved messages yet.'; savedList.append(empty); return;
    }
    state.saved.forEach((saved) => {
      const article = document.createElement('article'); article.className = 'c76-saved-item';
      const body = document.createElement('div'); const from = document.createElement('b'); from.textContent = saved.conversation; const copy = document.createElement('p'); copy.textContent = saved.copy; const meta = document.createElement('small'); meta.textContent = `${saved.time || 'Saved'} · local bookmark`; body.append(from, copy, meta);
      const actions = document.createElement('div'); actions.className = 'c76-saved-item__actions';
      const open = document.createElement('button'); open.type = 'button'; open.textContent = 'Open in chat'; open.addEventListener('click', () => { const conversation = conversations.find((item) => item.id === saved.conversationId); if (!conversation) return; savedView.hidden = true; threadPanel.inert = false; threadPanel.removeAttribute('inert'); threadPanel.removeAttribute('aria-hidden'); selectConversation(conversation, conversation.row); window.setTimeout(() => { const messageNode = messagesNode.querySelector(`[data-message-id="${CSS.escape(saved.messageId)}"]`); messageNode?.scrollIntoView({ block: 'center' }); messageNode?.classList.add('is-revealed'); window.setTimeout(() => messageNode?.classList.remove('is-revealed'), 1400); }, 0); });
      const remove = document.createElement('button'); remove.type = 'button'; remove.ariaLabel = 'Remove saved message'; remove.textContent = 'Remove';
      remove.addEventListener('click', () => { state.saved = state.saved.filter((item) => item.key !== saved.key); renderSaved(); showToast('Bookmark removed'); });
      actions.append(open, remove); article.append(body, actions); savedList.append(article);
    });
  };
  ...
  const syncConversationScrollbar = () => {
    ...
    const overflowing = conversationList.scrollHeight > conversationList.clientHeight + 1;
  ...
  const renderConversation = ({ focus = true } = {}) => {
    ...
    if (presence) presence.textContent = conversation.presence;
  ...
  root.querySelector('[data-saved-back]')?.addEventListener('click', () => { savedView.hidden = true; threadPanel.inert = false; threadPanel.removeAttribute('inert'); threadPanel.removeAttribute('aria-hidden'); activeConversation().row.focus(); });
  backButton?.addEventListener('click', () => { menu?.classList.add('is-expanded'); searchInput?.focus(); });
  root.querySelector('[data-menu-collapse]')?.addEventListener('click', () => { menu?.classList.remove('is-expanded'); activeConversation().row.focus(); });
  searchInput?.addEventListener('focus', () => { if (compact.matches) menu?.classList.add('is-expanded'); });
  ...
  root.querySelectorAll('[data-conversation-filter]').forEach((button) => button.addEventListener('click', () => {
    ...
    closePopovers(); renderList(); searchInput?.focus();
  }));
  ...
  root.querySelectorAll('[data-chat-menu]').forEach((button) => button.addEventListener('click', () => {
    ...
    if (action === 'mute') conversation.muted = !conversation.muted;
  ...
  messagesNode?.addEventListener('click', async (event) => {
    ...
    if (control.dataset.messageMenu && message) {
      ...
      if (action === 'react') { message.reactions ||= {}; message.reactions['💜'] = (message.reactions['💜'] || 0) + 1; renderMessages({ keepScroll: true }); }
      if (action === 'save') { const key = `${conversation.id}:${message.id}`; if (!state.saved.some((item) => item.key === key)) state.saved.push({ key, conversation: conversation.name, conversationId: conversation.id, messageId: message.id, copy: message.text || message.media?.title || 'Media message', time: message.time }); renderSaved(); showToast('Saved to Saved Messages'); }
    ...
    if (control.dataset.reaction && message) { message.reactions[control.dataset.reaction] += 1; renderMessages({ keepScroll: true }); return; }
    if (control.dataset.mediaOpen && message?.media) { openMedia(message.media, control); return; }
    if (control.hasAttribute('data-voice-play')) { const playing = control.getAttribute('aria-pressed') === 'true'; messagesNode.querySelectorAll('[data-voice-play]').forEach((item) => { item.setAttribute('aria-pressed', 'false'); item.textContent = '▶'; }); control.setAttribute('aria-pressed', playing ? 'false' : 'true'); control.textContent = playing ? '▶' : '❚❚'; announce(playing ? 'Voice preview paused.' : 'Voice preview playing locally.'); }
  });
  ...
  const attachmentConfig = {
    ...
    gallery: { kind: 'photo', accept: 'image/*', maxBytes: 12 * 1024 * 1024, label: 'gallery photo' },
  };
  ...
  attachmentInput?.addEventListener('change', () => {
    ...
    const isDocument = Boolean(documentMimes.includes(file.type) || ['.pdf', '.doc', '.docx', '.txt', '.rtf', '.xls', '.xlsx', '.csv'].includes(extension));
    const validType = config.kind === 'photo' ? isImage : config.kind === 'video' ? isVideo : isDocument;
    if (!validType) { showToast(`Choose a ${config.label} file.`); announce(`The selected file is not a valid ${config.label}.`); attachmentTrigger?.focus(); return; }
    if (file.size > config.maxBytes) { showToast(`This file is larger than ${formatBytes(config.maxBytes)}.`); announce('The selected file is too large.'); attachmentTrigger?.focus(); return; }
    const previous = activeConversation().attachment;
    disposeAttachmentPreview(previous);
    const localObjectUrl = URL.createObjectURL(file);
    const attachment = { kind: config.kind, src: localObjectUrl, title: file.name, size: file.size, mime: file.type || 'application/octet-stream', localObjectUrl: true };
  ...
  const updateVoiceRecorder = () => {
    ...
    if (voicePause) {
      ...
      if (icon) icon.textContent = voicePaused ? '▶' : 'Ⅱ';
    }
  };
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (!mediaViewer.hidden) { closeMedia(); return; }
    if (searchPanel && !searchPanel.hidden) { event.preventDefault(); closeThreadSearch(); return; }
    closePopovers(true);
    if (compact.matches && menu?.classList.contains('is-expanded')) { menu.classList.remove('is-expanded'); activeConversation().row.focus(); }
  }, { capture: true });
  ...
  document.addEventListener('click', (event) => {
    if (compact.matches && menu?.classList.contains('is-expanded') && !menu.contains(event.target)) menu.classList.remove('is-expanded');
    const openPopover = root.querySelector('.c76-chat-search:not([hidden]),.c76-chat-more-menu:not([hidden]),.c76-message-menu:not([hidden]),.c76-emoji-picker:not([hidden]),.c76-attachment-menu:not([hidden]),.c76-filter-menu:not([hidden]),.c76-account-menu:not([hidden])');
  ...
  compact.addEventListener?.('change', () => { menu?.classList.remove('is-expanded'); syncConversationScrollbar(); });
  ...
  renderSaved(); renderList(); renderConversation({ focus: false }); syncSuggestionNav(); syncScrollBottom();
})();
