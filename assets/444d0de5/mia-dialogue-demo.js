(() => {
  ...
  const render = (d) => {
    ...
    const runChanged = lastRunId !== null && lastRunId !== d.runId;
    lastRunId = d.runId;
    emit('nebula:mia-demo-snapshot', d);
    if (runChanged || d.events.at(-1)?.type === 'demo.reset') emit('nebula:mia-demo-reset', { runId: d.runId, expertId: d.expertId, demo: true });
    ...
    if (d.phase === 'connecting' && !connectionShown) {
      ...
      emit('nebula:mia-demo-connection-settled', { runId: d.runId, expertId: d.expertId, expertName: 'Mia Jacomo', outcome, statusAnchor: root.querySelector('[data-consultation-control]') || null, previousPhase: previousConnectionPhase });
      if (outcome) emit('nebula:mia-demo-availability', { runId: d.runId, expertId: d.expertId, expertName: 'Mia Jacomo', outcome, statusAnchor: root.querySelector('[data-consultation-control]') || null, demo: true, persisted: false, backendRequired: false });
    }
