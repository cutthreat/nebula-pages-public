(() => {
    ...
    const timerNotice = params.get('state') === 'paid_active'
        ? 'Paid timer is a static Figma sample; no billing.'
        : 'Active timer is local demo only; no billing.';
    root.innerHTML = `
        <strong>Fixture host</strong>
        <p>In-memory demo. No API, persistence or payment. ${timerNotice}</p>
        <label>Outcome
    ...
    const setupActiveChatFixture = () => {
        ...
        timer.textContent = '00:00:06';
        active.dataset.fixtureTimer = 'static-source-snapshot';
        active.dataset.sessionAuthority = 'backend-required';
    };
