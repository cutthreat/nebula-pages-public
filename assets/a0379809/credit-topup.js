(() => {
  ...
  const complete = (orderResult = null) => {
    if (orderResult) {
      setStep('success');
      if (successCopy) successCopy.textContent = `Sandbox order ${orderResult.order?.id || ''} was created. No provider was called and your balance is unchanged. Credits post only after an authorized payment is reconciled as settled.`;
      announce('Order created. Balance unchanged until provider settlement is reconciled.');
      root.dispatchEvent(new CustomEvent('nebula:credit-topup-completed', { bubbles: true, detail: { ...selectionDetail('order-created'), orderId: orderResult.order?.id || null, balanceBefore: currentBalance, balanceAfter: currentBalance, localBalanceChanged: false, serverBalanceMutation: false, persisted: true, sandbox: true } }));
      timer = null;
      return;
    }
  ...
  confirm.addEventListener('click', async () => {
    ...
    if (window.NebulaIntegration) {
      announce('Creating a sandbox order. No payment gateway is called.');
      timer = -1;
      try {
        const result = await window.NebulaIntegration.command('checkout.order.create', { packageId: detail.packageId }, { idempotencyKey: `credit-topup:${detail.packageId}:${Date.now()}` });
        complete(result);
      } catch (error) {
        timer = null;
        confirm.disabled = false;
        setStep('packages');
        announce(`Order was not created: ${error.message}`);
        root.dispatchEvent(new CustomEvent('nebula:integration-error', { bubbles: true, detail: { intent: 'nebula:credit-topup-confirm-required', command: 'checkout.order.create', code: error.code || 'SANDBOX_ERROR', message: error.message, sandbox: true } }));
      }
      return;
    }
    announce('Preparing local preview. No payment gateway was called.');
    timer = window.setTimeout(complete, 650);
  });
