(() => {
  ...
  const draftOwner = () => JSON.stringify([root.dataset.sessionId || '', root.dataset.expertId || '']);
  ...
  root.addEventListener('nebula:consultation-review-open', (event) => {
    ...
    const ownerProvided = payload.completedSessionId
    ...
    if (!ownerProvided || (root.dataset.staticProjection !== 'true' && !valid)) {
    ...
    // Save before changing identity: an unsent review belongs to its consultation/expert.
    drafts.set(draftOwner(), { text: field.value, rating: ratingDraft });
    ...
    const savedDraft = drafts.get(draftOwner());
    field.value = savedDraft?.text ?? (typeof payload.reviewDraft === 'string' ? payload.reviewDraft : '');
    ratingDraft = savedDraft?.rating ?? null;
