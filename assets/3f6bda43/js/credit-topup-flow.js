(() => {
  ...
  const packagePayload = (methodType = 'credit_card') => {
    ...
    return {
      ...
      capabilities: { checkoutSubmit: true, savedMethodChooser: true }, opener: selection.opener,
    };
  };
  const syncCopy = (root) => {
  ...
  const openReview = () => {
    ...
    open(review, 'nebula:checkout-review-open', packagePayload('credit_card'));
  };
  const openCard = () => open(card, 'nebula:card-details-open', {
    ...packagePayload('credit_card'), serverState: 'checkout_card_details', secureFormCapability: true,
  ...
  const openPaymentError = () => open(paymentError, 'nebula:payment-error-open', {
    ...
    capabilities: { alternatePaymentMethod: true, paymentSupport: true }, opener: selection.opener,
  });
  const openCardError = () => open(cardError, 'nebula:card-payment-error-open', {
    ...
    capabilities: { walletPaymentIntent: true, savedPaymentMethodIntent: true, paymentMethodChooser: true, paymentSupport: true }, opener: selection.opener,
  });

  // This is an explicitly local test payment, not ApplePaySession or a wallet charge.
  // Keep one order and stable command keys through retries, including lost receipts.
  const payWithApplePayTest = async () => {
    if (paymentPending) return;
    paymentPending = true;
    applePayAttempt ||= { key: `apple-pay-test:${crypto.randomUUID()}`, packageId: canonicalPackageId() };
    const attempt = applePayAttempt;
    hide(review);
    if (topup.hidden) closeAll();
    const showResult = (phase, detail = {}) => document.dispatchEvent(new CustomEvent('nebula:credit-topup-test-payment', {
      detail: { phase, opener: selection.opener, ...detail },
    }));
    showResult('processing');
    try {
      const api = window.NebulaIntegration;
      if (api?.mode !== 'local-file-backed-v1') throw new Error('Local payment sandbox is unavailable.');
      const created = await api.command('checkout.order.create', { packageId: attempt.packageId }, { idempotencyKey: `${attempt.key}:order` });
      const orderId = created.order.id;
      await api.command('checkout.payment.result', { orderId, result: 'authorized', providerReference: 'local-apple-pay-test' }, { idempotencyKey: `${attempt.key}:authorize` });
      const receipt = await api.command('checkout.order.reconcile', { orderId, providerStatus: 'settled' }, { idempotencyKey: `${attempt.key}:settle` });
      if (receipt.order?.status !== 'paid' || !Number.isFinite(receipt.balance)) throw new Error('The local payment is not reconciled.');
      showResult('success', { credits: receipt.order.credits, balance: receipt.balance, orderId });
    } catch (error) {
      // Do not report a decline or create another order when a response is unknown.
      showResult('error');
    } finally {
  ...
  const submitReview = () => {
    if (paymentPending || review.hidden) return;
    const active = review.querySelector('[role="radio"][aria-checked="true"]');
    if (active?.dataset.methodType === 'apple_pay') { payWithApplePayTest(); return; }
    hide(review);
  ...
  document.addEventListener('nebula:credit-topup-test-payment-retry', () => payWithApplePayTest());

  document.addEventListener('nebula:credit-topup-checkout-requested', (event) => {
    if (event.target !== topup) return;
    if (paymentPending) return;
    selection = { ...selection, ...event.detail, opener: event.detail?.opener instanceof HTMLElement ? event.detail.opener : null };
    applePayAttempt = null;
  ...
  document.addEventListener('nebula:alternate-payment-method-requested', (event) => {
    if (event.target === paymentError) { sandboxWorkflow('billing', 'order.review'); hide(paymentError); openReview(); }
  });
