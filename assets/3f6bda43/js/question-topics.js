(() => {
    ...
    const emitSelection = (control) => {
        ...
        if (iconSource) {
            // Reuse the paired C76 exports already rendered by this owner.
            icon.setAttribute('src', iconSource.replace(/g02-(?:un)?selected\.svg$/, `g02-${selected ? 'selected' : 'unselected'}.svg`));
        }
