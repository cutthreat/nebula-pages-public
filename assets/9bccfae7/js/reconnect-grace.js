(() => {
  ...
  root.addEventListener('nebula:consultation-reconnect-open', (event) => {
    ...
    const ownerProvided = detail.sessionId
      && detail.expertSnapshot?.displayName
      && detail.expertSnapshot?.imageUrl;
    ...
    if (!ownerProvided || (root.dataset.staticProjection !== 'true' && !requiredSnapshot)) {
    ...
    if (detail.expertSnapshot?.displayName) {
      ...
      if (avatar) {
        avatar.src = detail.expertSnapshot.imageUrl;
        avatar.alt = detail.expertSnapshot.displayName;
      }
    }
    root.hidden = false;
    frame.hidden = false;
