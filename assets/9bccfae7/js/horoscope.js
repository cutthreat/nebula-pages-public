(() => {
  ...
  const bioPrompts = {
    physical: 'Where would a steadier pace help today?',
    emotional: 'Which relationship needs more intention today?',
    intellectual: 'What idea deserves clear attention today?'
  };
  ...
  const updateBio = (button, announceUpdate = true) => {
    ...
    if (insight) insight.textContent = `${bioPrompts[focusKey]} Reflect with an advisor, not as health guidance.`;
