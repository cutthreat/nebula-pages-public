(() => {
  ...
  const conversations = rows
    ...
    .map((row) => {
      ...
      return {
        id: slug(name), name, row, avatar: rowProfiles[name]?.avatar, avatarWebp: rowProfiles[name]?.avatarWebp || '', unread: rowProfiles[name]?.unread || 0,
  ...
  const defaultConversation = conversations.find((conversation) => conversation.id === slug('Margo Lover'));
  if (isDefaultOpen && defaultConversation) {
    ...
    defaultConversation.presence = 'Offline';
  ...
  const state = { activeId: slug('Margo Lover'), filter: 'all', saved: seedSavedMessages, search: '', defaultOpen: isDefaultOpen };
  ...
  const renderMessages = ({ keepScroll = false } = {}) => {
    ...
    if (state.defaultOpen && conversation.id === slug('Margo Lover')) {
      root.dataset.messengerLive = 'false';
    ...
    // Leaving the source Default screen enters the separate live-history
    // interaction state; it must not inherit Default-screen geometry.
  ...
  const renderConversation = ({ focus = true } = {}) => {
    ...
    const psychicSlug = routedPsychicProfiles[conversation.name];
  ...
  const closeSavedView = ({ restoreFocus = false } = {}) => {
    ...
    if (compact.matches && restoreFocus) workspace?.classList.add('is-list-view');
  ...
  searchTrigger?.addEventListener('keydown', (event) => {
    if ((event.key === 'Enter' || event.key === ' ') && !searchShell?.classList.contains('is-search-open')) {
      event.preventDefault();
      openConversationSearch();
    }
  });
  compact.addEventListener?.('change', () => { if (!compact.matches) menu?.classList.remove('is-expanded'); syncCompactView(); syncConversationSearch(); });
  ...
  const validateConsultationHostSnapshot = (snapshot) => {
    ...
    if (snapshot.elapsedSeconds < consultationSeconds) return { valid: false, reason: 'regressive' };
    return { valid: true };
  };
  ...
  const renderConsultationControl = () => {
    ...
    const paused = consultationState === 'paused';
    const ended = consultationState === 'ended';
    const pending = consultationPendingIntent !== null || consultationState.startsWith('pending_');
    const canEnd = active || paused;
    consultationCluster?.setAttribute('data-consultation-state', projectedState);
    threadPanel?.setAttribute('data-consultation-visual-state', active ? 'active' : paused ? 'paused' : 'idle');
    ...
    if (consultationTimerNode) {
      consultationTimerNode.hidden = !(active || paused);
    ...
    consultationControls.forEach((control) => {
      ...
      control.dataset.consultationIntent = canEnd ? 'end' : 'start';
      control.disabled = pending || ended;
      control.setAttribute('aria-disabled', String(pending || ended));
      control.setAttribute('aria-pressed', String(active || paused));
      control.setAttribute('aria-expanded', 'false');
      const label = control.querySelector('[data-consultation-label]');
      const controlLabel = pending ? 'Consultation request pending service confirmation' : ended ? 'Paid consultation ended' : canEnd ? `End paid consultation, ${formatConsultationTime(consultationSeconds)}` : control.classList.contains('c76-consultation-action') ? 'Start paid consultation' : 'Start consultation';
  ...
  const openConsultationConfirmation = (opener) => {
    ...
    window.setTimeout(() => consultationConfirmation.querySelector('[data-consultation-confirm]')?.focus(), 0);
  };
  const emitConsultationIntent = (intent) => {
    if (consultationPendingIntent || !['start', 'end'].includes(intent)) return;
    consultationPendingIntent = intent;
    renderConsultationControl();
    root.dispatchEvent(new CustomEvent(intent === 'start' ? 'nebula:consultation-start-intent-required' : 'nebula:consultation-end-intent-required', { bubbles: true, detail: { backendRequired: true, sessionMutated: false, billingMutated: false, persisted: false, staticProjection: true, liveMount: false } }));
    announce(intent === 'start' ? 'Consultation request sent. Waiting for service confirmation.' : 'End request sent. Waiting for service confirmation.');
  };
  consultationControls.forEach((control) => control.addEventListener('click', () => {
    if (consultationPendingIntent || consultationState === 'ended') return;
    if (consultationState === 'active' || consultationState === 'paused') { emitConsultationIntent('end'); return; }
    openConsultationConfirmation(control);
  }));
  consultationConfirmation?.querySelectorAll('[data-consultation-cancel]').forEach((button) => button.addEventListener('click', () => closeConsultationConfirmation()));
  consultationConfirmation?.querySelector('[data-consultation-confirm]')?.addEventListener('click', () => { closeConsultationConfirmation({ restoreFocus: false }); emitConsultationIntent('start'); consultationControls[0]?.focus(); });
  root.addEventListener('nebula:consultation-host-snapshot', (event) => applyConsultationHostSnapshot(event.detail));
  renderConsultationControl();
  ...
  document.addEventListener('keydown', (event) => {
    if (consultationConfirmation && !consultationConfirmation.hidden) {
      ...
      if (event.key === 'Tab') {
        const focusables = Array.from(consultationConfirmation.querySelectorAll('button:not([disabled])')).filter((item) => !item.hidden);
