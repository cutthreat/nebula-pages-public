(() => {
  ...
  // A valid question asks the host to create one connection attempt. The
  // client does not create attempt identity, timing, outcome or a paid route.
  questionBeforeStart?.addEventListener('nebula:question-before-start-submit-required', (event) => {
    ...
    if (event.target !== questionBeforeStart || detail.staticProjection !== true || !target || !connectingExpert || connectionIntentPendingExpertId === target.id) return;
    connectionIntentPendingExpertId = target.id;
    questionBeforeStart.dispatchEvent(new CustomEvent('nebula:question-before-start-client-transition', {
      bubbles: true,
      detail: { selectedExpertId: target.id, staticProjection: true },
    }));
    connectingExpert.dispatchEvent(new CustomEvent('nebula:connection-attempt-selected-expert', {
      bubbles: true,
      detail: {
        expertId: target.id,
        expertName: target.displayName,
        statusAnchor: detail.trigger instanceof HTMLElement ? detail.trigger : root.querySelector('[data-psychic-start]'),
      },
    }));
    emit('nebula:connection-attempt-required', {
      expertId: target.id,
      expert: target.displayName,
      backendRequired: true,
