(() => {
  ...
  panel.innerHTML = `<details open><summary>DEMO · Mia / автоматический эксперт</summary><div class="mia-demo__body">
  ...
  const render = (d) => {
    ...
    if (d.phase === 'connecting' && !connectionShown) {
      ...
      emit('nebula:connection-presentation-open', {expertId: d.expertId, expertName: 'Mia Jacomo', avatar: root.querySelector('[data-chat-avatar]')?.currentSrc, durationMs: 30000, presentationOnly: true, demo: false});
    } else if (d.phase !== 'connecting' && connectionShown) {
      connectionShown = false;
      emit('nebula:mia-demo-connection-settled');
    }
  ...
  const run = (action, extra = {}) => {
    ...
    return work.finally(() => { if (action !== 'tick') userActionQueued = false; });
  };
